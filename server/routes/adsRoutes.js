...
async function getAdSetOwnerCoachId(adSetId) {
    const row = await get(`SELECT c.coach_id
       FROM ad_sets s
...
async function audit(req, action, entityType, entityId, oldState, newState) {
    try {
        await run(`INSERT INTO ad_audit_logs (actor_id, actor_role, action, entity_type, entity_id, old_state, new_state, ip_address)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [req.user.id, req.user.role, action, entityType, entityId,
...
router.get('/campaigns', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const isAdmin = req.user.role === 'admin';
        const coachFilter = isAdmin ? '' : 'WHERE c.coach_id = ?';
        const params = isAdmin ? [] : [req.user.id];
        const campaigns = await query(`SELECT c.*,
...
router.post('/campaigns', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        if (!(await adsEnabled()))
            return res.status(403).json({ message: 'Ads system is currently disabled' });
        ...
        if (!allowedOnCreate.has(status) && req.user.role !== 'admin')
            status = 'pending_review';
        const result = await run(`INSERT INTO ad_campaigns (coach_id, name, objective, daily_budget, lifetime_budget, budget_type, schedule_start, schedule_end, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [req.user.id, name, objective || 'coaching', daily_budget || 0, lifetime_budget || 0,
        ...
        if (status === 'pending_review') {
            await run(`INSERT INTO ad_moderation_reviews (campaign_id, status) VALUES (?, 'pending')`, [result.insertId]);
        }
...
router.get('/campaigns/:id', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        ...
        const ads = adSetIds.length
            ? await query(`SELECT a.*, ac.media_url, ac.format, ac.thumbnail_url FROM ads a LEFT JOIN ad_creatives ac ON ac.id = a.creative_id WHERE a.ad_set_id IN (${adSetIds.map(() => '?').join(',')})`, adSetIds)
            : [];
...
router.patch('/campaigns/:id', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const campaign = await get('SELECT * FROM ad_campaigns WHERE id = ?', [req.params.id]);
        if (!campaign)
        ...
        if (req.user.role !== 'admin' && campaign.coach_id !== req.user.id)
            return res.status(403).json({ message: 'Not your campaign' });
        const { name, objective, daily_budget, lifetime_budget, budget_type, schedule_start, schedule_end } = req.body;
        await run(`UPDATE ad_campaigns SET name=COALESCE(?,name), objective=COALESCE(?,objective),
       ...
       WHERE id=?`, [name, objective, daily_budget, lifetime_budget, budget_type, schedule_start, schedule_end, req.params.id]);
...
router.delete('/campaigns/:id', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const campaign = await get('SELECT * FROM ad_campaigns WHERE id = ?', [req.params.id]);
        if (!campaign)
            return res.status(404).json({ message: 'Not found' });
...
router.patch('/campaigns/:id/status', authenticateToken, adminOnly, async (req, res) => {
    try {
        const { status, admin_note } = req.body;
        const campaign = await get('SELECT * FROM ad_campaigns WHERE id = ?', [req.params.id]);
        if (!campaign)
            return res.status(404).json({ message: 'Not found' });
        await run(`UPDATE ad_campaigns SET status=?, admin_note=?, reviewed_by=?, reviewed_at=NOW(), updated_at=NOW() WHERE id=?`, [status, admin_note || null, req.user.id, req.params.id]);
        // Update moderation review
        await run(`UPDATE ad_moderation_reviews SET status=?, reviewer_id=?, notes=?, resolved_at=NOW() WHERE campaign_id=? AND status='pending'`, [status === 'active' ? 'approved' : status === 'rejected' ? 'rejected' : 'flagged', req.user.id, admin_note || null, req.params.id]);
        // Notify coach on rejection
        if (status === 'rejected' && campaign.coach_id) {
            const reason = admin_note ? `: ${admin_note}` : '';
            await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [campaign.coach_id, 'ad_rejected', '\u274c Campaign Rejected', `Your campaign "${campaign.name || 'Untitled'}" has been rejected${reason}`, '/coach/ads/my-ads']);
...
// AD SETS
// ─────────────────────────────────────────────────────────────────────────────
router.get('/campaigns/:campaignId/ad-sets', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
...
router.post('/campaigns/:campaignId/ad-sets', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        ...
        if (req.user.role !== 'admin' && campaign.coach_id !== req.user.id)
            return res.status(403).json({ message: 'Not your campaign' });
        const { name, placement, target_gender, target_age_min, target_age_max, target_location, target_lat, target_lng, target_radius_km, target_interests, target_activity_levels, target_languages, exclude_existing_clients, exclude_opted_out, daily_budget, } = req.body;
        const result = await run(`INSERT INTO ad_sets
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [req.params.campaignId, name || 'Ad Set 1', placement || 'feed',
...
router.patch('/ad-sets/:id', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
...
router.delete('/ad-sets/:id', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
...
// ADS
// ─────────────────────────────────────────────────────────────────────────────
router.get('/ad-sets/:adSetId/ads', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const ads = await query(`SELECT a.*, ac.media_url, ac.format, ac.thumbnail_url
       FROM ads a LEFT JOIN ad_creatives ac ON ac.id = a.creative_id
       WHERE a.ad_set_id = ? ORDER BY a.id`, [req.params.adSetId]);
        res.json({ ads });
    }
    catch (err) {
        res.status(500).json({ message: err.message });
...
router.post('/ad-sets/:adSetId/ads', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const { name, creative_id, headline, body, cta, destination_type, destination_ref, campaign_id, variant_group, is_control } = req.body;
        const result = await run(`INSERT INTO ads (ad_set_id, campaign_id, name, creative_id, headline, body, cta, destination_type, destination_ref, variant_group, is_control)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [req.params.adSetId, campaign_id, name || 'Ad 1', creative_id || null,
...
router.patch('/ads/:id', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        ...
        await run(`UPDATE ads SET name=COALESCE(?,name), headline=COALESCE(?,headline), body=COALESCE(?,body),
         cta=COALESCE(?,cta), status=COALESCE(?,status), creative_id=COALESCE(?,creative_id), updated_at=NOW()
       WHERE id=?`, [name, headline, body, cta, status, creative_id, req.params.id]);
...
router.delete('/ads/:id', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
...
// CREATIVES
// ─────────────────────────────────────────────────────────────────────────────
router.get('/creatives', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const isAdmin = req.user.role === 'admin';
...
router.post('/creatives', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
...
router.post('/creatives/upload', authenticateToken, coachOrAdmin, upload.single('file'), optimizeImage(), async (req, res) => {
    try {
        if (!req.file)
...
// ── Delete creative ──
router.delete('/creatives/:id', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
...
router.get('/ads', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const isAdmin = req.user.role === 'admin';
        const filter = isAdmin ? '' : 'WHERE c.coach_id = ?';
        const params = isAdmin ? [] : [req.user.id];
        const ads = await query(`SELECT a.*, s.name AS ad_set_name, c.name AS campaign_name,
...
router.get('/analytics/stats', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
...
router.get('/analytics/daily', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const coachId = req.user.role === 'admin' ? req.query.coach_id : req.user.id;
        const window = parseInt(req.query.window) || 30;
        const analytics = await query(`SELECT DATE(e.recorded_at) AS date, e.event_type,
              COUNT(*) AS count
       FROM ad_events e
       ...
       ORDER BY date`, coachId ? [coachId, window] : [window]);
        // Pivot into per-day objects
        const dayMap = {};
        for (const row of analytics) {
            const d = row.date;
            if (!dayMap[d])
                dayMap[d] = { date: d, impressions: 0, clicks: 0, conversions: 0 };
            if (row.event_type === 'impression')
                dayMap[d].impressions = row.count;
            else if (row.event_type === 'click')
                dayMap[d].clicks = row.count;
            else if (row.event_type === 'conversion')
                dayMap[d].conversions = row.count;
        }
...
router.post('/events', authenticateToken, async (req, res) => {
    try {
        ...
        if (!ad_id || !campaign_id || !event_type)
            return res.status(400).json({ message: 'Missing required fields' });
        await run(`INSERT INTO ad_events (ad_id, campaign_id, event_type, user_id, session_id, placement) VALUES (?, ?, ?, ?, ?, ?)`, [ad_id, campaign_id, event_type, req.user.id, session_id || null, placement || null]);
        ...
        if (col)
            await run(`UPDATE ads SET ${col} = ${col} + 1 WHERE id = ?`, [ad_id]);
        res.json({ ok: true });
    }
    catch (err) {
        res.status(500).json({ message: err.message });
...
router.get('/analytics/summary', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const isAdmin = req.user.role === 'admin';
        const window = parseInt(req.query.window) || 30;
       ...
       ${filter}`, params);
        const byDay = await query(`SELECT DATE(recorded_at) AS day, event_type, COUNT(*) AS cnt
       FROM ad_events e
       JOIN ad_campaigns c ON c.id = e.campaign_id
       WHERE ${coachId ? 'c.coach_id = ? AND' : ''}
         e.recorded_at >= DATE_SUB(NOW(), INTERVAL ? DAY)
       GROUP BY day, event_type ORDER BY day`, coachId ? [coachId, window] : [window]);
        const topAds = await query(`SELECT a.id, a.name, a.impressions, a.clicks, a.conversions, a.ctr, c.name AS campaign_name
       FROM ads a
       JOIN ad_sets s ON s.id = a.ad_set_id
       JOIN ad_campaigns c ON c.id = s.campaign_id
       ${coachId ? 'WHERE c.coach_id = ?' : ''}
       ORDER BY a.impressions DESC LIMIT 5`, coachId ? [coachId] : []);
...
// WALLET
// ─────────────────────────────────────────────────────────────────────────────
router.get('/wallet', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
...
router.post('/wallet/credit', authenticateToken, adminOnly, async (req, res) => {
    try {
        const { coach_id, amount, note } = req.body;
        if (!coach_id || !amount)
            return res.status(400).json({ message: 'coach_id and amount required' });
        // Upsert wallet
        await run(`INSERT INTO ad_wallets (coach_id, balance) VALUES (?, ?) ON DUPLICATE KEY UPDATE balance = balance + ?`, [coach_id, amount, amount]);
        const wallet = await get('SELECT * FROM ad_wallets WHERE coach_id = ?', [coach_id]);
        await run(`INSERT INTO ad_wallet_ledger (coach_id, entry_type, amount, balance_after, note, created_by) VALUES (?, 'credit', ?, ?, ?, ?)`, [coach_id, amount, wallet.balance, note || null, req.user.id]);
...
// ─────────────────────────────────────────────────────────────────────────────
router.get('/placements', authenticateToken, async (_req, res) => {
    try {
...
router.get('/audit-logs', authenticateToken, adminOnly, async (req, res) => {
    try {
        ...
        const logs = await query(`SELECT l.*, u.name AS actor_name, u.email AS actor_email
...
// TEMPLATE PRESETS
// ─────────────────────────────────────────────────────────────────────────────
router.get('/presets', authenticateToken, coachOrAdmin, async (_req, res) => {
    try {
...
router.get('/coach-ads', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const isAdmin = req.user.role === 'admin';
        const where = isAdmin ? '' : 'WHERE a.coach_id = ?';
        const params = isAdmin ? [] : [req.user.id];
        const ads = await query(`SELECT a.*,
...
router.patch('/coach-ads/:id/status', authenticateToken, adminOnly, async (req, res) => {
    try {
        const { status, admin_note } = req.body;
        ...
        await run(`UPDATE coach_ads SET status = ?, admin_note = ?, updated_at = NOW() WHERE id = ?`, [status, admin_note || null, req.params.id]);
        // If approving, also approve the payment status and set boost window
        if (status === 'active') {
            await run(`UPDATE coach_ads SET payment_status = 'approved',
         boost_start = COALESCE(boost_start, NOW()),
         boost_end = COALESCE(boost_end, COALESCE(schedule_end, DATE_ADD(NOW(), INTERVAL 30 DAY)))
         WHERE id = ? AND (payment_status IS NULL OR payment_status != 'approved')`, [req.params.id]);
            await run(`UPDATE ad_payments SET status = 'approved', updated_at = NOW() WHERE ad_id = ? AND status = 'pending'`, [req.params.id]);
        }
        // Notify coach on rejection
...
//# sourceMappingURL=adsRoutes.js.map