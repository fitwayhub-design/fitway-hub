...
export async function saveSmtpSettings(s) {
...
export async function sendMail(opts) {
    ...
    await run(`INSERT INTO emails (account_id, sender, recipient, subject, text_body, html_body, direction, message_id)
     VALUES (?, ?, ?, ?, ?, ?, 'outbound', ?)`, [opts.fromAccountId, account.email, opts.to, opts.subject, opts.text || '', opts.html || '', info.messageId || '']);
    return info;
}
...
export async function sendSystemEmail(opts) {
    const settings = await getSmtpSettings();
    if (!settings || !settings.smtp_host || !settings.enabled)
        return false;
    try {
        const transporter = createTransporter(settings);
            ...
            try {
                ...
                for (const addr of toList) {
                    ...
                    for (const a of addresses) {
                        ...
                        if (acct) {
                            await run(`INSERT INTO emails (account_id, sender, recipient, subject, text_body, html_body, direction)
                 VALUES (?, ?, ?, ?, ?, ?, 'inbound')`, [acct.id, from, email, subject, textBody, htmlBody]);
                        }
                    }
                }
                callback();
