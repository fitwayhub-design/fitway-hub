...
function parseTiffForDate(buf, tiff) {
    ...
    const readIfd = (ifdOff, depth) => {
        if (depth > 3 || ifdOff + 2 > buf.length)
            return null;
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (tag === 0x8769)
                exifPtr = tiff + u32(e + 8); // Exif sub-IFD pointer
        }
        if (exifPtr)
            return readIfd(exifPtr, depth + 1);
        return null;
    };
    try {
        return readIfd(tiff + u32(tiff + 4), 0);
    }
    catch {
        return null;
    }
}
export function extractImageCaptureTime(buf) {
    if (buf.length < 4 || buf[0] !== 0xff || buf[1] !== 0xd8)
        return null; // JPEG SOI
    let offset = 2;
    while (offset + 4 < buf.length) {
        ...
        if (marker === 0xda)
            break; // start of scan — metadata done
...
export function extractVideoCaptureTime(buf) {
    ...
    const findBox = (start, end, type) => {
        ...
        while (o + 8 <= end) {
            ...
            if (size < header)
                break;
            if (t === type)
                return { start: o + header, end: o + size };
            o += size;
        }
        return null;
    };
    try {
        ...
        if (+d < Date.UTC(2000, 0, 1) || +d > Date.now() + 86400000)
            return null; // sanity
        return isNaN(+d) ? null : d;
    }
    catch {
...
export function extractCaptureTime(buf, mime) {
    try {
        if (mime.startsWith('image/'))
            return extractImageCaptureTime(buf);
        if (mime.startsWith('video/'))
            return extractVideoCaptureTime(buf);
    }
 ...
 */
export function stampMediaCapture(req, _res, next) {
    try {
        const files = [];
        if (req.file)
            files.push(req.file);
