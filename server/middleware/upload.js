...
const fontFilter = (req, file, cb) => {
    ...
    const okMime = file.mimetype.startsWith('font/') ||
            ...
            for (const file of files) {
                ...
                if (ext === '.gif') {
                    try {
                        ...
                        if (meta.format !== 'gif')
                            return next(new Error('Invalid image: bytes do not match the declared format'));
                    }
                    catch {
...
async function validateVideoSize(req, res, next) {
    try {
        ...
        try {
            const rows = await dbQuery('SELECT setting_value FROM app_settings WHERE setting_key = ?', ['max_video_upload_size_mb']);
