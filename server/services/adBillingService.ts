...
export async function billImpression(adId: number): Promise<void> {
  try {
    const ad = await get<any>(
    ...
    console.warn('billImpression error:', (err as Error).message);
  }
}

/** Deduct click cost (CPC = 3× CPM rate). Updates reach estimate. */
export async function billClick(adId: number): Promise<void> {
  try {
    const ad = await get<any>(
    ...
    console.warn('billClick error:', (err as Error).message);
  }
}

/** Check daily budget reset at midnight and schedule expiry */
export async function expireAndResetAds(): Promise<void> {
  try {
    ...
    console.warn('expireAndResetAds error:', (err as Error).message);
  }
}
