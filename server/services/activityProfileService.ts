...
function deriveActivityLevel(avgSteps: number, workoutsPerWeek: number): string {
...
export async function updateUserActivityProfile(userId: number): Promise<void> {
  try {
    const avgRow = await get<any>(
      `SELECT IFNULL(ROUND(AVG(steps)),0) as avg_steps
       FROM steps_entries
       WHERE user_id = ? AND date >= DATE_SUB(CURDATE(), INTERVAL 13 DAY)`,
      [userId]
    );
    const avgSteps: number = Number(avgRow?.avg_steps) || 0;

    // Workouts in last 30 days
    let recentWorkouts = 0;
    try {
      const wRow = await get<any>(
    ...
    console.warn('updateUserActivityProfile error:', (err as Error).message);
  }
}

let lastSweepMs = 0;
export async function maybeSweepAllUsers(): Promise<void> {
  const now = Date.now();
  if (now - lastSweepMs < 60 * 60 * 1000) return;
  lastSweepMs = now;
  try {
