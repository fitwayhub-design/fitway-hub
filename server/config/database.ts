...
function parseSslFromUrl(params: URLSearchParams): mysql.SslOptions | undefined {
  ...
  if (!DB_SSL && !wantsSsl) return undefined;
  return {
...
function getConnectionConfig(includeDatabase: boolean): mysql.PoolOptions {
...
async function seedDefaultAccounts() {
  ...
  for (const acc of accounts) {
    const existing = await get<any>('SELECT id, role FROM users WHERE email = ?', [acc.email]);
    if (!existing) {
      ...
      await run(
        ...
        [acc.email, hash, acc.name, acc.role, avatar,
          acc.role === 'user' ? 1 : 0,
          acc.role !== 'user'  ? 1 : 0,
...
export async function seedDefaultAppSettings() {
  const defaults: [string, string, string, string, string][] = [
    ...
    ['secondary_color', '#FF7A6E', 'color', 'branding', 'Secondary Color (Dark mode)'],
    ['secondary_color_light', '#2563EB', 'color', 'branding', 'Secondary Color (Light mode)'],
...
async function seedDefaultBlogs() {
  ...
  const author = await get<any>("SELECT id FROM users WHERE role IN ('coach','admin') ORDER BY id ASC LIMIT 1");
  if (!author) return; // No users yet — will run again on next restart
  ...
  const existing = await get('SELECT COUNT(*) as cnt FROM blog_posts WHERE language = ?', ['en']) as any;
  if (existing?.cnt >= 10) return; // Already seeded
  ...
  for (const blog of BLOGS) {
    try {
      ...
      const enRow = await get<any>('SELECT id FROM blog_posts WHERE slug=? AND language=?', [blog.slug, 'en']);
      await p.execute(
...
export async function initDatabase() {
  ...
  try { await run('DELETE FROM revoked_tokens WHERE expires_at < NOW()'); } catch {}
  ...
  try { await run("DELETE FROM app_settings WHERE setting_key IN ('feature_user_chat','feature_coach_chat')"); } catch {}
