...
async function startServer() {
  ...
  const PUBLIC_META: Record<string, { title: string; desc: string }> = {
  ...
  const injectMeta = (html: string, meta: { title: string; desc: string; url?: string; image?: string; type?: string }) => {
    const tags = [
  ...
  app.get('/robots.txt', (_req: any, res: any) => {
    ...
    res.type('text/plain').send(
      ['User-agent: *', 'Allow: /', 'Disallow: /app/', 'Disallow: /admin/', 'Disallow: /coach/',
  ...
  app.get('/sitemap.xml', async (_req: any, res: any) => {
    try {
      const base = siteBase();
      if (!base) return res.status(404).send('Set APP_BASE_URL to enable the sitemap');
      const staticPaths = Object.keys(PUBLIC_META);
      let posts: any[] = [];
      try {
        posts = await dbQuery(
          "SELECT slug, updated_at FROM blog_posts WHERE status = 'published' ORDER BY COALESCE(published_at, created_at) DESC LIMIT 500"
        );
  ...
  if (existsSync(distIndex)) {
    ...
    app.get('/blogs/:slug', async (req: any, res: any, next: any) => {
      try {
        const html = await readIndex();
        const post: any = await dbGet(
          "SELECT title, excerpt, header_image_url, slug FROM blog_posts WHERE slug = ? AND status = 'published'",
          [String(req.params.slug || '')]
        );
        if (!post) return res.send(injectMeta(html, { ...PUBLIC_META['/blogs'], url: siteBase() ? `${siteBase()}/blogs` : undefined }));
        const base = siteBase();
    ...
    for (const [route, meta] of Object.entries(PUBLIC_META)) {
      app.get(route, async (_req: any, res: any, next: any) => {
        try {
          const base = siteBase();
