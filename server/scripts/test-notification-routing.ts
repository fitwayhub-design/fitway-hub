...
async function main() {
  ...
  const user = await get<any>('SELECT id, name, email, last_active FROM users WHERE id = ?', [userId]);
  if (!user) {
  ...
  if (!top) { console.error('FAIL: no notification was created'); process.exit(3); }
  if (!top.link) { console.error('FAIL: top notification has null link — Layer A regressed'); process.exit(4); }
