...
async function uniqueSlug(base, ignoreId, language = 'en') {
    ...
    while (true) {
        ...
        if (!row || (ignoreId && Number(row.id) === Number(ignoreId)))
            return slug;
        slug = `${safeBase}-${i++}`;
    }
}
...
export const getPublicBlogs = async (req, res) => {
    try {
        ...
        const posts = await query(`SELECT bp.id, bp.title, bp.slug, bp.excerpt, bp.content, bp.header_image_url, bp.video_url,
...
export const getPublicBlogBySlug = async (req, res) => {
    try {
        ...
        const post = Number.isFinite(byId) && byId > 0
            ? await get(`SELECT bp.*, u.name AS author_name, u.avatar AS author_avatar
         FROM blog_posts bp
         LEFT JOIN users u ON u.id = bp.author_id
         WHERE bp.id = ? AND bp.language = ? AND bp.status = 'published'`, [byId, lang])
            : await get(`SELECT bp.*, u.name AS author_name, u.avatar AS author_avatar
         FROM blog_posts bp
         LEFT JOIN users u ON u.id = bp.author_id
         WHERE bp.slug = ? AND bp.language = ? AND bp.status = 'published'`, [key, lang]);
        if (!post)
...
export const createBlog = async (req, res) => {
    try {
        ...
        const { insertId } = await run(`INSERT INTO blog_posts
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [title, slug, excerpt, content, headerImage, video, videoDuration, language, relatedBlogId, status, userId, role, publishedAt]);
        const post = await get(`SELECT bp.*, u.name AS author_name, u.avatar AS author_avatar
       FROM blog_posts bp
       LEFT JOIN users u ON u.id = bp.author_id
       WHERE bp.id = ?`, [insertId]);
...
export const updateBlog = async (req, res) => {
    try {
       ...
       WHERE id = ?`, [title, slug, excerpt, content, nextHeaderImage, nextVideo, videoDuration, relatedBlogId, status, publishedAt, postId]);
        const post = await get(`SELECT bp.*, u.name AS author_name, u.avatar AS author_avatar
       FROM blog_posts bp
       LEFT JOIN users u ON u.id = bp.author_id
       WHERE bp.id = ?`, [postId]);
