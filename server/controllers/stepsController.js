...
export const addSteps = async (req, res) => {
    try {
        ...
        if (typeof steps !== 'number' || steps < 0 || steps > 100000)
            return res.status(400).json({ message: 'Steps must be between 0 and 100000' });
        const existing = await get('SELECT id FROM steps_entries WHERE user_id = ? AND date = ?', [userId, date]);
        if (existing) {
            await run('UPDATE steps_entries SET steps = ?, calories_burned = ?, distance_km = ?, notes = ?, tracking_mode = ? WHERE user_id = ? AND date = ?', [steps, caloriesBurned || null, distanceKm || null, notes || null, trackingMode || 'manual', userId, date]);
            return res.json({ message: 'Steps updated successfully', date, steps });
        }
        else {
            const { insertId } = await run('INSERT INTO steps_entries (user_id, date, steps, calories_burned, distance_km, notes, tracking_mode) VALUES (?, ?, ?, ?, ?, ?, ?)', [userId, date, steps, caloriesBurned || null, distanceKm || null, notes || null, trackingMode || 'manual']);
