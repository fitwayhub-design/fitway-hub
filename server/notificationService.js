...
async function getFcmAccessToken() {
    ...
    if (_fcmTokenCache && Date.now() < _fcmTokenCache.exp)
...
export async function registerPushToken(userId, token, platform = 'android') {
    await run(`INSERT INTO push_tokens (user_id, token, platform) VALUES (?, ?, ?)
     ON DUPLICATE KEY UPDATE token = VALUES(token), updated_at = NOW()`, [userId, token, platform]);
}
/** Remove a user's push token (e.g. on logout) */
export async function removePushToken(userId, platform = 'android') {
 ...
 */
export function linkForTemplate(slug) {
    switch (slug) {
...
export async function sendPushToUser(userId, title, body, templateId, link = '/', type = 'info') {
    ...
    for (const t of tokens) {
        ...
        await run('INSERT INTO push_log (user_id, template_id, title, body, status, error_message) VALUES (?,?,?,?,?,?)', [userId, templateId || null, title, body, ok ? 'sent' : 'failed', ok ? null : 'FCM delivery failed']);
        if (ok)
            anySuccess = true;
    }
...
export async function sendPushFromTemplate(userId, slug, vars = {}, link, type) {
...
export async function sendPushToSegment(title, body, segment = 'all', templateId, link = '/', type = 'info') {
    ...
    if (segment === 'users')
        ...
        sql += " WHERE u.role = 'coach'";
    else if (segment === 'inactive')
        sql += ' WHERE u.last_active < DATE_SUB(NOW(), INTERVAL 7 DAY)';
    ...
    for (const r of rows) {
        ...
        await run('INSERT INTO push_log (user_id, template_id, title, body, status) VALUES (?,?,?,?,?)', [r.user_id, templateId || null, t, b, ok ? 'sent' : 'failed']);
...
export async function createInAppNotification(userId, type, title, body, link) {
    await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [userId, type, title, body, link || null]);
}
...
export async function runScheduledPushes() {
    ...
    for (const tier of tiers) {
        try {
            const users = await query(`SELECT DISTINCT pt.user_id, u.name
         FROM push_tokens pt
         ...
         LIMIT 100`, [tier.days, tier.maxDays, tier.slug]);
            for (const user of users) {
                const firstName = (user.name || '').split(' ')[0] || 'there';
                await sendPushFromTemplate(user.user_id, tier.slug, { first_name: firstName });
            }
            if (users.length)
                console.log(`[Push] Sent ${tier.slug} to ${users.length} users`);
        }
        catch (err) {
...
export async function sendWelcomeMessages(userId, role, name, email) {
