...
router.post('/bootstrap-admin', async (req: any, res: Response) => {
  try {
    ...
    const user = await get<any>('SELECT id, email, role FROM users WHERE email = ?', [email]);
...
router.delete('/users/:id', authenticateToken, adminOnly, async (req: any, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      const result: any = await run('DELETE FROM users WHERE id = ?', [userId]);
...
router.patch('/videos/:id/approval', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
    ...
    const existing = await get<any>('SELECT * FROM workout_videos WHERE id = ?', [req.params.id]);
    if (!existing) return res.status(404).json({ message: 'Video not found' });

    await run(
      'UPDATE workout_videos SET approval_status = ?, rejection_reason = ?, approved_by = ?, approved_at = ?, updated_at = NOW() WHERE id = ?',
      [status, status === 'rejected' ? (reason || '') : null, req.user.id, status === 'approved' ? new Date() : null, req.params.id]
    );
    ...
    const video = await get<any>('SELECT * FROM workout_videos WHERE id = ?', [req.params.id]);
...
router.patch('/ads/:id/status', authenticateToken, adminOnly, async (req: any, res: Response) => {
  ...
  try {
    const ad = await get<any>('SELECT * FROM coach_ads WHERE id = ?', [id]);
    if (status === 'active') {
...
router.patch('/ads/:id/payment', authenticateToken, adminOnly, async (req: any, res: Response) => {
  ...
  try {
    ...
    if (payment_status === 'approved') {
      ...
      const payment = await get<any>('SELECT amount, duration_minutes FROM ad_payments WHERE ad_id = ? AND status = ?', [id, 'approved']);
      ...
      if (totalMinutes > 0) {
        ...
        const ad = await get<any>('SELECT schedule_start, schedule_end, daily_budget, total_budget, budget_type FROM coach_ads WHERE id = ?', [id]);
    ...
    if (payment_status === 'rejected') {
      ...
      const ad = await get<any>('SELECT coach_id, title FROM coach_ads WHERE id = ?', [id]);
      if (ad?.coach_id) {
...
async function seedAdsForCoach(coachId: number) {
  ...
  const u = (id: string) => `https://images.unsplash.com/${id}?w=800&auto=format&fit=crop&q=70`;
  const IMG = {
...
router.put('/moderator-permissions', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
    const perms = req.body?.permissions || {};
    const value = JSON.stringify(perms);
    const existing: any = await get("SELECT id FROM app_settings WHERE setting_key = 'moderator_permissions'");
    if (existing) {
      await run("UPDATE app_settings SET setting_value = ? WHERE setting_key = 'moderator_permissions'", [value]);
    } else {
      await run(
        "INSERT INTO app_settings (setting_key, setting_value, setting_type, category, label) VALUES ('moderator_permissions', ?, 'json', 'access', 'Moderator permissions')",
        [value]
      );
    }
    res.json({ message: 'Moderator permissions saved', permissions: perms });
  } catch { res.status(500).json({ message: 'Failed to save moderator permissions' }); }
});
...
router.get('/moderator-permissions/:userId', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
  ...
  } catch { res.status(500).json({ message: 'Failed to load moderator access' }); }
});

router.put('/moderator-permissions/:userId', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
    ...
    if (typeof perms !== 'object') return res.status(400).json({ message: 'permissions must be an object or null' });
    const value = JSON.stringify(perms);
    const existing: any = await get("SELECT user_id FROM moderator_user_permissions WHERE user_id = ?", [userId]);
    if (existing) {
  ...
  } catch { res.status(500).json({ message: 'Failed to save moderator access' }); }
});

// Community moderation endpoints
router.get('/community/posts', authenticateToken, modPerm('community_view'), async (_req: any, res: Response) => {
  try {
...
router.patch('/community/posts/:id/hide', authenticateToken, modPerm('community_moderate'), async (req: any, res: Response) => {
  try {
...
// ── Community stats ──────────────────────────────────────────────────────────
router.get('/community/stats', authenticateToken, modPerm('community_view'), async (_req: any, res: Response) => {
  try {
  ...
  } catch { res.status(500).json({ message: 'Failed to fetch community stats' }); }
});

// ── Community challenges (admin) ──────────────────────────────────────────────
router.get('/community/challenges', authenticateToken, modPerm('challenges_view'), async (_req: any, res: Response) => {
  try {
    ...
    res.json({ challenges });
  } catch { res.status(500).json({ message: 'Failed to fetch challenges' }); }
});

router.post('/community/challenges', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
    const { title, description, start_date, end_date, image_url } = req.body || {};
    if (!title || !String(title).trim()) return res.status(400).json({ message: 'Title is required' });
    const ins: any = await run(
      'INSERT INTO challenges (creator_id, title, description, start_date, end_date, image_url) VALUES (?,?,?,?,?,?)',
      [req.user.id, String(title).trim(), description || null, start_date || null, end_date || null, image_url || null]
    );
    res.json({ message: 'Challenge created', id: ins?.insertId });
  } catch { res.status(500).json({ message: 'Failed to create challenge' }); }
});

router.patch('/community/challenges/:id', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
    const { title, description, start_date, end_date, image_url } = req.body || {};
    await run(
      'UPDATE challenges SET title = COALESCE(?, title), description = ?, start_date = ?, end_date = ?, image_url = ? WHERE id = ?',
      [title != null ? String(title).trim() : null, description ?? null, start_date ?? null, end_date ?? null, image_url ?? null, req.params.id]
    );
    res.json({ message: 'Challenge updated' });
  } catch { res.status(500).json({ message: 'Failed to update challenge' }); }
});
...
router.get('/community/comments', authenticateToken, modPerm('community_view'), async (_req: any, res: Response) => {
  try {
...
router.get('/certification-requests', authenticateToken, adminOnly, async (_req: any, res: Response) => {
  try {
    ...
    res.json({ requests });
  } catch { res.status(500).json({ message: 'Failed to fetch certification requests' }); }
});

// Approve or reject a certification request
router.patch('/certification-requests/:id', authenticateToken, adminOnly, async (req: any, res: Response) => {
  const { action, admin_notes } = req.body;
  if (!['approve', 'reject'].includes(action)) return res.status(400).json({ message: 'Invalid action' });
  try {
    const request: any = await get('SELECT * FROM certification_requests WHERE id = ?', [req.params.id]);
    ...
    if (action === 'approve') {
      ...
      await run('UPDATE certification_requests SET status = ?, admin_notes = ?, reviewed_by = ?, reviewed_at = ? WHERE id = ?',
  ...
  } catch (err) { console.error(err); res.status(500).json({ message: 'Failed to update certification request' }); }
});

// Admin can also directly grant/revoke (legacy endpoint)
router.patch('/users/:id/certification', authenticateToken, adminOnly, async (req: any, res: Response) => {
  ...
  } catch { res.status(500).json({ message: 'Failed to update certification' }); }
});

// ── Coach reports moderation ───────────────────────────────────────────────
router.get('/coach-reports', authenticateToken, adminOnly, async (_req: any, res: Response) => {
  try {
...
router.patch('/coach-reports/:id', authenticateToken, adminOnly, async (req: any, res: Response) => {
  const { status, admin_notes } = req.body || {};
  if (!['resolved', 'dismissed'].includes(status)) {
    return res.status(400).json({ message: 'Status must be resolved or dismissed' });
  }
  try {
    const report: any = await get('SELECT * FROM coach_reports WHERE id = ?', [req.params.id]);
    ...
    await run(
      ...
      [status, admin_notes ? String(admin_notes).trim().slice(0, 3000) : null, req.user.id, req.params.id]
    );
...
router.get('/dashboard-config', async (_req: Request, res: Response) => {
  try {
    const rows = await query("SELECT setting_key, setting_value, setting_type FROM app_settings WHERE category = 'dashboard'") as any[];
...
router.post('/upload-dashboard-image', authenticateToken, adminOnly, multerToJson(uploadBranding.single('image')), optimizeImage(), async (req: any, res: Response) => {
  try {
    if (!req.file) return res.status(400).json({ message: 'No image file provided' });
...
// ── Font file upload ─────────────────────────────────────────────────────────
router.post('/upload-font', authenticateToken, adminOnly, uploadFont.single('font'), verifyUploadBytes('font'), async (req: any, res: Response) => {
  try {
...
router.get('/backup/database', authenticateToken, adminOnly, async (_req: any, res: Response) => {
  try {
    const pool = getPool();
    ...
    for (const tableRow of tables) {
      ...
      const [createRows] = await pool.execute(`SHOW CREATE TABLE \`${tableName}\``) as any[];
      ...
      // INSERT rows
      const [rows] = await pool.execute(`SELECT * FROM \`${tableName}\``) as any[];
      if (Array.isArray(rows) && rows.length > 0) {
        ...
        const valueRows = (rows as any[]).map(row =>
...
router.post('/backup/restore', authenticateToken, adminOnly, upload.single('file'), async (req: any, res: Response) => {
  try {
...
router.post('/generate-fake-users', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
    ...
    const BRACKETS: [number,number][] = [[18,24],[25,34],[35,44],[45,54],[55,65]];
      ...
      try {
        const result: any = await dbRun(
          `INSERT INTO users (name,email,password,role,is_premium,gender,height,weight,
           ...
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
          [name, email, fakeH, 'user', i%3===0?1:0, gender, h, w,
...
router.delete('/fake-users', authenticateToken, adminOnly, async (_req: any, res: Response) => {
  try {
    const fakeUsers = await query<any>(
    ...
    const ids = fakeUsers.map((u: any) => Number(u.id)).filter((id: number) => id > 0);
    const placeholders = ids.map(() => '?').join(',');
