...
// are both caught.
// ─────────────────────────────────────────────────────────────────────────────
export const CONTACT_INFO_MESSAGE = 'For your safety, sharing contact details (phone numbers, emails, social handles or links) is not allowed here. Please remove them and try again.';
...
export function containsContactInfo(raw) {
    ...
    if (/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}/i.test(deobfuscated))
        return true;
    ...
    if (/\d{8,}/.test(collapsed))
        return true;
    // 4) Messaging / social apps named explicitly — a strong signal someone is
    //    trying to move the conversation off-platform.
    if (/\b(whats\s*app|whatsapp|tele\s*gram|telegram|t\.me|insta\s*gram|instagram|snap\s*chat|snapchat|messenger|fb\s*messenger|viber|signal\s+app|wechat|\bimo\b)\b/i.test(text))
        return true;
    // 5) Links / URLs.
    if (/(https?:\/\/|www\.)\S+/i.test(original))
        return true;
    // 6) Social-style @handle (e.g. "@john_fit"). Require it to look like a
    //    handle (3+ chars, letters/digits/._) to avoid blocking "@everyone".
    if (/(^|[\s(])@[a-z0-9](?:[a-z0-9._]{2,})/i.test(original))
        return true;
    return false;
}
//# sourceMappingURL=contentGuard.js.map