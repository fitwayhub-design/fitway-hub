...
export const authenticateToken = async (req, res, next) => {
    ...
    try {
        ...
        let revoked = null;
        try {
            revoked = await get('SELECT id FROM revoked_tokens WHERE token_hash = ? AND user_id = ?', [tokenHash, decoded.id]);
        }
        catch (dbErr) {
...
export const requireActiveCoachMembershipForDeals = async (req, res, next) => {
    try {
        if (req.user?.role === 'admin')
            return next();
        if (req.user?.role !== 'coach')
            return next();
        const userRow = await get('SELECT role, coach_membership_active, membership_paid, created_at FROM users WHERE id = ?', [req.user.id]);
        if (!userRow)
