...
export interface GoalTypeMeta {
  key: GoalTypeKey;
  label: string;
  /** Short "how it's tracked" line shown to creators and athletes. */
  tracking: string;
  /** Verification methods a submission against this goal may use. */
  methods: string[];
  /** Unit of target_value ('' when the goal has no numeric target). */
  unit: string;
  /** Whether the creator must provide a positive numeric target. */
  needsTarget: boolean;
  /** Daily goals count one submission per calendar day. */
  daily: boolean;
  /** Weight goals measure progress from the first weigh-in (baseline). */
  weighIn?: boolean;
  /** Training goals reference a training from the admin-curated list. */
  needsTraining?: boolean;
  /** Walk/Run goals carry an activity ('walk' | 'run'). */
...
export const GOAL_TYPE_KEYS = Object.keys(GOAL_TYPES) as GoalTypeKey[];
...
export function isGoalType(v: any): v is GoalTypeKey {
...
export function methodsForGoals(goals: Array<{ goal_type: string }>): string[] {
  const out: string[] = [];
  for (const g of goals) {
    const meta = GOAL_TYPES[g.goal_type as GoalTypeKey];
    if (!meta) continue;
    for (const m of meta.methods) if (!out.includes(m)) out.push(m);
  }
  return out;
}

/** Sensible bounds for creator-entered targets, per goal type. */
export function clampGoalTarget(type: GoalTypeKey, value: number): number {
...
export function defaultGoalTitle(g: {
  goal_type: GoalTypeKey; target_value?: number; activity?: string | null; training_title?: string | null;
}): string {
...
/** The minimal slice of a submission row the progress math needs. */
export interface GoalSubmission {
  goal_id: number | null;
  method: string;
  metric_value: number;
  activity_date: string; // YYYY-MM-DD
  status: string;        // approved | auto_approved | pending | rejected
}
...
export interface GoalProgress {
  ...
  /** Transformation only. `*_pending` = submitted, awaiting review. */
  has_before?: boolean;
  has_after?: boolean;
  before_pending?: boolean;
  after_pending?: boolean;
  /** Daily goals: whether an entry exists for `today` (when provided). */
  done_today?: boolean;
}
...
export function computeGoalProgress(
  goal: { id: number; goal_type: string; target_value: number },
  submissions: GoalSubmission[],
  today?: string,
): GoalProgress {
  const meta = GOAL_TYPES[goal.goal_type as GoalTypeKey];
  ...
  const out: GoalProgress = { current: 0, target, pct: 0, completed: false };
  if (!meta) return out;
...
export function overallGoalsProgress(
  goals: Array<{ id: number; goal_type: string; target_value: number }>,
  submissions: GoalSubmission[],
): { pct: number; completedGoals: number; perGoal: GoalProgress[] } {
  if (!goals.length) return { pct: 0, completedGoals: 0, perGoal: [] };
