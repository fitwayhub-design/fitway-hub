...
import {
  METHODS, METRICS, methodsForType, isMethodAllowed, clampMetric,
  basePointsForSubmission, recomputeParticipant, recomputeAll, aliasForParticipant,
} from '../services/challengeScoring.js';
import {
  GOAL_TYPES, GOAL_TYPE_KEYS, isGoalType, methodsForGoals, clampGoalTarget,
  defaultGoalTitle, computeGoalProgress, GoalSubmission, GoalTypeKey,
  GOAL_COMPLETION_TARGET,
} from '../services/challengeGoalTypes.js';

// ── helpers ──────────────────────────────────────────────────────────────────
const uid = (req: Request) => Number((req as any).user?.id);
const role = (req: Request) => String((req as any).user?.role || 'user');
const isAdmin = (req: Request) => ['admin', 'moderator'].includes(role(req));
...
function computeState(c: any): string {
  if (['draft', 'pending_review', 'rejected', 'cancelled', 'finalized'].includes(c.status)) return c.status;
  const now = Date.now();
...
function canManage(c: any, req: Request): boolean {
  return isAdmin(req) || Number(c.creator_id) === uid(req);
}

function parseMethods(input: any): string[] {
  if (Array.isArray(input)) return input.map(String);
  if (typeof input === 'string' && input.trim()) return input.split(',').map(s => s.trim()).filter(Boolean);
  return [];
}

async function defaultRewardTiers(): Promise<any> {
  const row = await get<any>("SELECT setting_value FROM app_settings WHERE setting_key = 'challenge_reward_defaults'");
  try { return JSON.parse(row?.setting_value || '{}'); } catch { return {}; }
}

/** A challenge's goal list (joined with training titles for display). */
async function loadGoals(challengeId: number): Promise<any[]> {
  const rows = await query<any>(
    `SELECT g.*, t.title AS training_title
     FROM challenge_goals g LEFT JOIN trainings t ON t.id = g.training_id
     WHERE g.challenge_id = ? ORDER BY g.position ASC, g.id ASC`,
    [challengeId],
  );
  return rows.map(g => ({
    ...g,
    methods: GOAL_TYPES[g.goal_type as GoalTypeKey]?.methods || [],
    tracking: GOAL_TYPES[g.goal_type as GoalTypeKey]?.tracking || '',
  }));
}
 ...
 */
async function parseGoalsInput(input: any): Promise<{ goals?: any[]; error?: string }> {
  let raw: any;
  if (input == null || input === '') return { goals: [] };
  if (typeof input === 'string') {
    try { raw = JSON.parse(input); } catch { return { error: 'Invalid goals payload.' }; }
  } else raw = input;
  if (!Array.isArray(raw)) return { error: 'Goals must be a list.' };
  if (raw.length > MAX_GOALS) return { error: `A challenge can have at most ${MAX_GOALS} goals.` };

  const goals: any[] = [];
  for (const [i, g] of raw.entries()) {
    const type = String(g?.goal_type || '');
    if (!isGoalType(type)) return { error: `Goal ${i + 1}: unknown goal type.` };
    const meta = GOAL_TYPES[type];

    const target = clampGoalTarget(type, Number(g?.target_value) || 0);
    if (meta.needsTarget && target <= 0) return { error: `Goal ${i + 1} (${meta.label}): a positive target is required.` };

    let trainingId: number | null = null;
    let trainingTitle: string | null = null;
    if (meta.needsTraining) {
      trainingId = Number(g?.training_id) || 0;
      if (!trainingId) return { error: `Goal ${i + 1} (${meta.label}): pick a training from the list.` };
      const t = await get<any>('SELECT id, title FROM trainings WHERE id = ?', [trainingId]);
      if (!t) return { error: `Goal ${i + 1} (${meta.label}): that training no longer exists.` };
    ...
    if (meta.needsActivity) {
      ...
      if (!activity) return { error: `Goal ${i + 1} (${meta.label}): choose walking or running.` };
    }

    const customTitle = String(g?.title || '').trim().slice(0, 160);
    const description = String(g?.description || '').trim().slice(0, 500) || null;
    if (containsContactInfo(customTitle) || containsContactInfo(description || '')) return { error: CONTACT_INFO_MESSAGE };
...
async function insertGoals(challengeId: number, goals: any[]) {
  for (const g of goals) {
    await run(
      `INSERT INTO challenge_goals (challenge_id, position, goal_type, title, description, training_id, activity, target_value, target_unit)
       VALUES (?,?,?,?,?,?,?,?,?)`,
      [challengeId, g.position, g.goal_type, g.title, g.description, g.training_id, g.activity, g.target_value, g.target_unit],
    );
  }
}

async function refreshCount(challengeId: number) {
  const r = await get<any>(`SELECT COUNT(*) c FROM challenge_participants WHERE challenge_id = ? AND status = 'active'`, [challengeId]);
...
async function isServedAthlete(coachId: number, athleteId: number): Promise<boolean> {
  const r = await get<any>(
    `SELECT 1 AS ok FROM users u WHERE u.id = ? AND (
     ...
     OR EXISTS(SELECT 1 FROM coach_follows f WHERE f.coach_id = ? AND f.follower_id = u.id))`,
    [athleteId, coachId, coachId, coachId],
  );
  return !!r;
}
...
export const listChallenges = async (req: Request, res: Response) => {
  try {
    ...
    if (q) list = list.filter(c => String(c.title || '').toLowerCase().includes(q.toLowerCase()));
...
export const listMyInvitations = async (req: Request, res: Response) => {
  try {
    const me = uid(req);
    const rows = await query<any>(
      `SELECT i.id AS invitation_id, i.token, i.status AS invite_status, i.created_at AS invited_at,
...
export const getChallenge = async (req: Request, res: Response) => {
  try {
    ...
    const c = await get<any>(
      `SELECT c.*, u.name AS creator_name, u.avatar AS creator_avatar FROM challenges c
       LEFT JOIN users u ON c.creator_id = u.id WHERE c.id = ? AND c.deleted_at IS NULL`,
      [req.params.id],
    );
    if (!c) return res.status(404).json({ message: 'Challenge not found' });

    const me_part = await get<any>('SELECT * FROM challenge_participants WHERE challenge_id = ? AND user_id = ?', [c.id, me]);
    const invited = await get<any>("SELECT id, token FROM challenge_invitations WHERE challenge_id = ? AND invitee_id = ? AND status='sent'", [c.id, me]);
    ...
    const counts = await get<any>(`SELECT COUNT(*) total FROM challenge_participants WHERE challenge_id = ? AND status='active'`, [c.id]);
    let pendingReview = 0;
    if (canManage(c, req)) {
      const pr = await get<any>(`SELECT COUNT(*) n FROM challenge_submissions WHERE challenge_id = ? AND status='pending'`, [c.id]);
    ...
    res.json({
      challenge: {
        ...
        reward_tiers: (() => { try { return JSON.parse(c.reward_tiers || '{}'); } catch { return {}; } })(),
...
export const createChallenge = async (req: Request, res: Response) => {
  try {
    const me = uid(req);
    const r = role(req);
    if (!['coach', 'admin'].includes(r)) return res.status(403).json({ message: 'Only coaches and admins can create challenges.' });

    const b = req.body || {};
    const type = r === 'coach' ? 'team' : (b.type === 'team' ? 'team' : 'community');
    const title = String(b.title || '').trim();
    const description = String(b.description || '').trim();
    if (title.length < 3 || title.length > 80) return res.status(400).json({ message: 'Title must be 3–80 characters.' });
    if (containsContactInfo(title) || containsContactInfo(description)) return res.status(400).json({ message: CONTACT_INFO_MESSAGE });

    const startAt = b.start_at ? new Date(b.start_at) : null;
    const endAt = b.end_at ? new Date(b.end_at) : null;
    if (!startAt || isNaN(+startAt) || !endAt || isNaN(+endAt)) return res.status(400).json({ message: 'Valid start and end dates are required.' });
    if (+endAt <= +startAt) return res.status(400).json({ message: 'End date must be after the start date.' });
    ...
    if (parsedGoals.error) return res.status(400).json({ message: parsedGoals.error });
    ...
    if (goalRows.length) {
      ...
      if (methods.length === 0) methods = type === 'team' ? ['coach_approval'] : ['manual_checkin', 'photo_evidence'];
    }
    ...
    let status: string;
    if (r === 'admin') status = +startAt > Date.now() ? 'scheduled' : 'active';
    else status = (participantLimit > 0 && participantLimit <= autoLimit) || participantLimit === 0 && 100 <= autoLimit
    ...
    if (goalRows.length) await insertGoals(insertId, goalRows);
...
export const updateChallenge = async (req: Request, res: Response) => {
  try {
    const c = await get<any>('SELECT * FROM challenges WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
    if (!c) return res.status(404).json({ message: 'Challenge not found' });
    if (!canManage(c, req)) return res.status(403).json({ message: 'Not authorized' });
    const state = computeState(c);
    if (['finalized', 'cancelled'].includes(state)) return res.status(400).json({ message: 'A finalized or cancelled challenge cannot be edited.' });
    ...
    if (b.title != null) {
      ...
      if (t.length < 3 || t.length > 80) return res.status(400).json({ message: 'Title must be 3–80 characters.' });
      if (containsContactInfo(t)) return res.status(400).json({ message: CONTACT_INFO_MESSAGE });
      sets.push('title = ?'); vals.push(t);
    }
    if (b.description != null) {
      if (containsContactInfo(String(b.description))) return res.status(400).json({ message: CONTACT_INFO_MESSAGE });
      sets.push('description = ?'); vals.push(String(b.description));
    }
    if (b.rules_terms != null) { sets.push('rules_terms = ?'); vals.push(String(b.rules_terms).slice(0, 4000)); }
    ...
    if (b.reward_title != null) {
      ...
      if (containsContactInfo(rt)) return res.status(400).json({ message: CONTACT_INFO_MESSAGE });
      sets.push('reward_title = ?'); vals.push(rt || null);
    }
    if (b.reward_description != null) {
      const rd = String(b.reward_description).trim().slice(0, 500);
      if (containsContactInfo(rd)) return res.status(400).json({ message: CONTACT_INFO_MESSAGE });
      sets.push('reward_description = ?'); vals.push(rd || null);
    }
    if (b.reward_points != null) { sets.push('reward_points = ?'); vals.push(Math.min(100000, Math.max(0, parseInt(b.reward_points, 10) || 0))); }
    const files = (req as any).files as Express.Multer.File[] | undefined;
    if (files && files.length) { const url = await uploadToR2(files[0], 'challenges'); sets.push('image_url = ?'); vals.push(url); }

    // end_at: extend-only while running; free before start; admin override.
    if (b.end_at != null) {
      const ne = new Date(b.end_at);
      if (isNaN(+ne)) return res.status(400).json({ message: 'Invalid end date.' });
      if (running && !admin && +ne < +new Date(c.end_at)) return res.status(400).json({ message: 'A running challenge can only be extended, not shortened.' });
      sets.push('end_at = ?'); vals.push(ne); sets.push('end_date = ?'); vals.push(ne.toISOString().slice(0, 10));
    }
    ...
    if (b.participant_limit != null) {
      ...
      if (running && !admin && nl !== 0 && nl < Number(c.participant_limit)) return res.status(400).json({ message: 'Participant limit can only be raised while running.' });
      sets.push('participant_limit = ?'); vals.push(nl);
    }
    ...
    if (!locked) {
      ...
      if (b.goals_json != null) {
        ...
        goalsReplaced = true;
        if (parsed.goals?.length) { sets.push('verification_methods = ?'); vals.push(methodsForGoals(parsed.goals).join(',')); }
      }
      if (b.goals_json == null && b.verification_methods != null) {
        ...
        if (m.length) { sets.push('verification_methods = ?'); vals.push(m.join(',')); }
      }
    }

    if (!sets.length && !goalsReplaced) return res.status(400).json({ message: 'Nothing to update.' });
    ...
    if (running) {
      const parts = await query<any>(`SELECT user_id FROM challenge_participants WHERE challenge_id = ? AND status='active'`, [c.id]);
      for (const p of parts) notify(p.user_id, 'challenge_updated', 'Challenge updated', `“${c.title}” was updated.`, `/app/challenges`);
    }
    if (locked || goalsReplaced) await recomputeAll(c.id);
...
export const deleteChallenge = async (req: Request, res: Response) => {
  try {
    ...
    const parts = await query<any>(`SELECT user_id FROM challenge_participants WHERE challenge_id = ? AND status='active'`, [c.id]);
    for (const p of parts) notify(p.user_id, 'challenge_cancelled', 'Challenge cancelled', `“${c.title}” was cancelled.`, '/app/challenges');
...
// INVITES (team)
// ─────────────────────────────────────────────────────────────────────────────
export const getInvitableAthletes = async (req: Request, res: Response) => {
  try {
    const c = await get<any>('SELECT * FROM challenges WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
    if (!c) return res.status(404).json({ message: 'Challenge not found' });
    if (!canManage(c, req)) return res.status(403).json({ message: 'Not authorized' });
    ...
    const rows = await query<any>(
      ...
      [coachId, coachId, coachId, c.id, c.id],
    );
...
export const inviteParticipants = async (req: Request, res: Response) => {
  try {
    const c = await get<any>('SELECT * FROM challenges WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
    if (!c) return res.status(404).json({ message: 'Challenge not found' });
    if (!canManage(c, req)) return res.status(403).json({ message: 'Not authorized' });
    if (c.type !== 'team') return res.status(400).json({ message: 'Only team challenges use invitations.' });

    const ids: number[] = Array.isArray(req.body?.userIds) ? req.body.userIds.map(Number).filter(Boolean) : [];
    if (!ids.length) return res.status(400).json({ message: 'No athletes selected.' });

    let invited = 0; const rejected: number[] = [];
    for (const athleteId of ids) {
      if (!isAdmin(req) && !(await isServedAthlete(Number(c.creator_id), athleteId))) { rejected.push(athleteId); continue; }
      const token = randomBytes(24).toString('hex');
      const r = await run(
        `INSERT IGNORE INTO challenge_invitations (challenge_id, inviter_id, invitee_id, token) VALUES (?,?,?,?)`,
        [c.id, uid(req), athleteId, token],
      );
      if (r.affectedRows) {
        invited++;
...
export const respondInvitation = async (req: Request, res: Response) => {
  try {
    const me = uid(req);
    const { token, accept } = req.body || {};
    const inv = await get<any>('SELECT * FROM challenge_invitations WHERE challenge_id = ? AND token = ?', [req.params.id, String(token || '')]);
    // Tokens are single-use AND user-bound: a forwarded link fails for anyone else.
    if (!inv || Number(inv.invitee_id) !== me) return res.status(403).json({ message: 'This invitation is not valid for your account.' });
    if (inv.status !== 'sent') return res.status(400).json({ message: 'This invitation was already used.' });
...
// JOIN / LEAVE
// ─────────────────────────────────────────────────────────────────────────────
async function joinInternal(challengeId: number, userId: number, body: any, res: Response, skipEligibility: boolean) {
  const c = await get<any>('SELECT * FROM challenges WHERE id = ? AND deleted_at IS NULL', [challengeId]);
  if (!c) return res.status(404).json({ message: 'Challenge not found' });
  const state = computeState(c);
  if (!['scheduled', 'active'].includes(state)) return res.status(400).json({ message: 'This challenge is not open for joining.' });
  ...
  if (openUntil && Date.now() > openUntil) return res.status(400).json({ message: 'Entry for this challenge has closed.' });

  // Capacity.
  if (Number(c.participant_limit) > 0) {
    const cnt = await get<any>(`SELECT COUNT(*) n FROM challenge_participants WHERE challenge_id = ? AND status='active'`, [challengeId]);
    if ((cnt?.n || 0) >= Number(c.participant_limit)) return res.status(400).json({ message: 'This challenge is full.' });
  }

  // Eligibility (community open join). Invites skip this.
  if (!skipEligibility) {
    if (c.type === 'team') return res.status(403).json({ message: 'Team challenges are invitation-only.' });
    const u = await get<any>('SELECT is_premium, created_at FROM users WHERE id = ?', [userId]);
    if (c.premium_only && !u?.is_premium) return res.status(403).json({ message: 'This challenge is for premium members only.' });
    if (Number(c.min_account_age_days) > 0 && u?.created_at) {
      const ageDays = (Date.now() - new Date(u.created_at).getTime()) / 86400000;
      if (ageDays < Number(c.min_account_age_days)) return res.status(403).json({ message: 'Your account is too new to join this challenge.' });
    }
  }
  ...
  const existing = await get<any>('SELECT * FROM challenge_participants WHERE challenge_id = ? AND user_id = ?', [challengeId, userId]);
  if (existing) {
    if (existing.status === 'active') return res.json({ message: 'Already joined.' });
    if (existing.status === 'removed') return res.status(403).json({ message: 'You were removed from this challenge.' });
    if (existing.status === 'left') {
      if (openUntil && Date.now() > openUntil) return res.status(400).json({ message: 'Entry has closed — you cannot rejoin.' });
      if (Number(existing.rejoin_count) >= 2) return res.status(400).json({ message: 'Rejoin limit reached.' });
      await run(
        "UPDATE challenge_participants SET status='active', left_at=NULL, rejoin_count=rejoin_count+1, streak=0, display_mode=?, alias=?, accepted_terms_at=NOW() WHERE id = ?",
        [displayMode, alias, existing.id],
      );
  ...
  await run(
    `INSERT INTO challenge_participants (challenge_id, user_id, display_mode, alias, status, accepted_terms_at)
     VALUES (?,?,?,?, 'active', NOW())`,
    [challengeId, userId, displayMode, alias],
  );
  await refreshCount(challengeId);
  if (Number(c.creator_id) !== userId) notify(Number(c.creator_id), 'challenge_joined', 'New participant', `Someone joined “${c.title}”.`, '/coach/challenges');
  res.json({ message: 'Joined the challenge.' });
}

export const joinChallenge = (req: Request, res: Response) => joinInternal(Number(req.params.id), uid(req), req.body, res, false);

export const leaveChallenge = async (req: Request, res: Response) => {
  try {
    const me = uid(req);
    const p = await get<any>("SELECT * FROM challenge_participants WHERE challenge_id = ? AND user_id = ? AND status='active'", [req.params.id, me]);
    if (!p) return res.status(404).json({ message: 'You are not in this challenge.' });
...
export const removeParticipant = async (req: Request, res: Response) => {
  try {
    ...
    if (!c) return res.status(404).json({ message: 'Challenge not found' });
    if (!canManage(c, req)) return res.status(403).json({ message: 'Not authorized' });
    const p = await get<any>('SELECT * FROM challenge_participants WHERE id = ? AND challenge_id = ?', [req.params.pid, c.id]);
...
export const submitEvidence = async (req: Request, res: Response) => {
  try {
    ...
    const c = await get<any>('SELECT * FROM challenges WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
    if (!c) return res.status(404).json({ message: 'Challenge not found' });
    const p = await get<any>("SELECT * FROM challenge_participants WHERE challenge_id = ? AND user_id = ? AND status='active'", [c.id, me]);
    if (!p) return res.status(403).json({ message: 'Join the challenge before submitting progress.' });

    const state = computeState(c);
    if (state !== 'active') return res.status(400).json({ message: state === 'scheduled' ? 'The challenge has not started yet.' : 'The challenge is closed for submissions.' });

    const method = String(req.body?.method || '');
    if (!METHODS[method]) return res.status(400).json({ message: 'Unknown verification method.' });
    ...
    if (goals.length) {
      ...
      if (!goal) return res.status(400).json({ message: 'Pick which goal this progress is for.' });
      if (!goal.methods.includes(method)) return res.status(400).json({ message: `“${goal.title}” is tracked by: ${goal.tracking}.` });
    } else {
      const allowed = parseMethods(c.verification_methods);
      if (!allowed.includes(method)) return res.status(400).json({ message: 'This verification method is not allowed for this challenge.' });
    }
    ...
    if (method === 'manual_step') { const r = clampMetric('steps', metricValue); metricValue = r.value; if (r.clamped) addFlag('step count clamped to plausible max'); }
    if (method === 'manual_distance') { const r = clampMetric('distance_km', metricValue); metricValue = r.value; if (r.clamped) addFlag('distance clamped to plausible max'); }
    if (method === 'gps_steps') { const r = clampMetric('steps', metricValue); metricValue = r.value; if (r.clamped) addFlag('GPS step count clamped to plausible max'); }
    ...
    if (goal) {
      if (goal.goal_type === 'walk_run') {
        ...
        metricValue = r.value;
        if (r.clamped) addFlag('distance clamped to plausible max');
        if (metricValue <= 0) return res.status(400).json({ message: 'Enter the distance you covered (km).' });
        ...
        const dupPhoto = await get<any>(
          `SELECT id FROM challenge_submissions WHERE participant_id = ? AND goal_id = ? AND method = ? AND status <> 'rejected' AND deleted_at IS NULL`,
          [p.id, goal.id, method],
        );
        if (dupPhoto) return res.status(400).json({ message: `You already submitted your ${method === 'before_photo' ? 'before' : 'after'} photo.` });
      }
    }
    if (method === 'weigh_in' && goal) { // weigh_in is only reachable through a weight goal
      if (!(metricValue >= 20 && metricValue <= 400)) return res.status(400).json({ message: 'Enter your current weight in kg (20–400).' });
      // One weigh-in per day keeps the trend honest.
      const dupWeigh = await get<any>(
        `SELECT id FROM challenge_submissions WHERE participant_id = ? AND goal_id = ? AND method='weigh_in' AND activity_date = ? AND status <> 'rejected' AND deleted_at IS NULL`,
        [p.id, goal.id, activityDate],
      );
      if (dupWeigh) return res.status(400).json({ message: 'You already logged a weigh-in today.' });
    }

    // One check-in per day (per goal when goal-based).
    if (method === 'manual_checkin') {
      const dup = goal
        ? await get<any>(`SELECT id FROM challenge_submissions WHERE participant_id = ? AND goal_id = ? AND method='manual_checkin' AND activity_date = ? AND status <> 'rejected' AND deleted_at IS NULL`, [p.id, goal.id, activityDate])
        : await get<any>(`SELECT id FROM challenge_submissions WHERE challenge_id = ? AND participant_id = ? AND method='manual_checkin' AND activity_date = ? AND status <> 'rejected' AND deleted_at IS NULL`, [c.id, p.id, activityDate]);
      if (dup) return res.status(400).json({ message: 'You already checked in for this day.' });
    }

    // Evidence handling: hash + dedupe.
    let evidenceUrl: string | null = null; let evidenceHash: string | null = null;
    const files = (req as any).files as Express.Multer.File[] | undefined;
    const file = files?.[0];
    if (meta.needsEvidence) {
      if (!file) return res.status(400).json({ message: 'This method requires a photo/video/screenshot.' });
      evidenceHash = createHash('sha256').update(file.buffer).digest('hex');
      const dupHash = await get<any>('SELECT id FROM challenge_submissions WHERE challenge_id = ? AND evidence_hash = ? AND deleted_at IS NULL', [c.id, evidenceHash]);
      if (dupHash) return res.status(400).json({ message: 'This exact file was already submitted.' });
    ...
    const { insertId } = await run(
      `INSERT INTO challenge_submissions
       ...
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [c.id, p.id, me, goal ? goal.id : null, method, metricValue, durationSeconds, evidenceUrl, evidenceHash, String(req.body?.note || '').slice(0, 500) || null, activityDate, status, trust, awarded, flagged ? 1 : 0, flagReason, capturedAt, geoLat, geoLng],
    );
    ...
    if (status === 'pending') notify(Number(c.creator_id), 'challenge_review', 'Submission to review', `New evidence in “${c.title}”.`, '/coach/challenges');
...
export const listSubmissions = async (req: Request, res: Response) => {
  try {
    ...
    const rows = await query<any>(
      `SELECT s.*, u.name AS user_name, u.avatar AS user_avatar, g.title AS goal_title, g.goal_type
...
async function reviewSubmission(req: Request, res: Response, approve: boolean) {
  const c = await get<any>('SELECT * FROM challenges WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
  if (!c) return res.status(404).json({ message: 'Challenge not found' });
  if (!canManage(c, req)) return res.status(403).json({ message: 'Not authorized' });
  const s = await get<any>('SELECT * FROM challenge_submissions WHERE id = ? AND challenge_id = ?', [req.params.sid, c.id]);
...
export const approveSubmission = (req: Request, res: Response) => reviewSubmission(req, res, true).catch(() => res.status(500).json({ message: 'Failed' }));
export const rejectSubmission = (req: Request, res: Response) => reviewSubmission(req, res, false).catch(() => res.status(500).json({ message: 'Failed' }));

// ─────────────────────────────────────────────────────────────────────────────
// LEADERBOARD / PROGRESS
// ─────────────────────────────────────────────────────────────────────────────
function displayName(row: any, viewerId: number, reveal: boolean) {
  if (Number(row.user_id) === viewerId) return { name: row.user_name || 'You', avatar: row.user_avatar, you: true, link_id: null };
  if (reveal || row.display_mode === 'real') return { name: row.user_name || 'Athlete', avatar: row.user_avatar, you: false, link_id: row.display_mode === 'real' || reveal ? row.user_id : null };
...
export const getLeaderboard = async (req: Request, res: Response) => {
  try {
    ...
    const c = await get<any>('SELECT * FROM challenges WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
    if (!c) return res.status(404).json({ message: 'Challenge not found' });
    const myPart = await get<any>('SELECT id FROM challenge_participants WHERE challenge_id = ? AND user_id = ?', [c.id, me]);
    if (c.type === 'team' && !canManage(c, req) && !myPart) return res.status(403).json({ message: 'Private leaderboard.' });
    ...
    if (scope === 'overall') {
      rows = await query<any>(
        `SELECT cp.*, u.name AS user_name, u.avatar AS user_avatar
      ...
      rows = await query<any>(
        `SELECT cp.id, cp.user_id, cp.display_mode, cp.alias, cp.joined_at, cp.best_streak, cp.progress_value,
...
export const getProgress = async (req: Request, res: Response) => {
  try {
    ...
    const goals = await loadGoals(c.id);
    const goalBased = goals.length > 0;
    const p = await get<any>('SELECT * FROM challenge_participants WHERE challenge_id = ? AND user_id = ?', [c.id, me]);
    if (!p) return res.json({ participant: null, submissions: [], rank: null, goals: goals.map(g => ({ ...g, progress: null })) });
    ...
    const subs = await query<any>('SELECT * FROM challenge_submissions WHERE participant_id = ? AND deleted_at IS NULL ORDER BY created_at DESC', [p.id]);
    const rk = await get<any>(
      `SELECT COUNT(*)+1 AS rank FROM challenge_participants
       WHERE challenge_id = ? AND status='active' AND (verified_points > ? OR (verified_points = ? AND progress_value > ?))`,
      [c.id, p.verified_points, p.verified_points, p.progress_value],
    );
    const today = todayInTz(c.timezone);
    const goalsWithProgress = goals.map(g => ({
      ...g,
      progress: computeGoalProgress(g, subs as GoalSubmission[], today),
    }));
    res.json({
      participant: p,
...
// ─────────────────────────────────────────────────────────────────────────────
export const finalizeChallenge = async (req: Request, res: Response) => {
  try {
    const c = await get<any>('SELECT * FROM challenges WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
    if (!c) return res.status(404).json({ message: 'Challenge not found' });
    if (!canManage(c, req)) return res.status(403).json({ message: 'Not authorized' });
    if (c.status === 'finalized') return res.status(400).json({ message: 'Already finalized.' });
    const force = String(req.query.force || '') === '1' && isAdmin(req);
    if (!force && c.end_at && Date.now() < Date.parse(c.end_at)) return res.status(400).json({ message: 'The challenge has not ended yet.' });
    ...
    res.json({ message: 'Challenge finalized and rewards granted.' });
  } catch (e) {
...
// ADMIN
// ─────────────────────────────────────────────────────────────────────────────
export const adminListChallenges = async (req: Request, res: Response) => {
  try {
...
export const adminReviewChallenge = async (req: Request, res: Response) => {
  try {
    ...
    if (approve) {
      ...
      if (req.body?.reward_tiers) { sets.push('reward_tiers = ?'); vals.push(JSON.stringify(req.body.reward_tiers)); }
      vals.push(c.id);
      await run(`UPDATE challenges SET ${sets.join(', ')} WHERE id = ?`, vals);
...
export const saveRewardSettings = async (req: Request, res: Response) => {
  try {
    const value = JSON.stringify(req.body?.settings || {});
    await run(
      `INSERT INTO app_settings (setting_key, setting_value, setting_type, category, label)
...
export const adminListReports = async (_req: Request, res: Response) => {
  try {
    const rows = await query<any>(
      `SELECT r.*, c.title AS challenge_title FROM challenge_reports r
...
export const getGoalOptions = async (_req: Request, res: Response) => {
  try {
    const trainings = await query<any>('SELECT id, title FROM trainings ORDER BY sort_order ASC, title ASC');
    res.json({
...
export const getUserRewards = async (req: Request, res: Response) => {
  try {
    ...
    const rows = await query<any>(
      `SELECT g.*, c.title AS challenge_title FROM challenge_reward_grants g
