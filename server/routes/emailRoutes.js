...
// ── Get mail domain ─────────────────────────────────────────────
router.get('/domain', authenticateToken, adminOnly, (_req, res) => {
    res.json({ domain: getMailDomain() });
});
// ── SMTP Settings ───────────────────────────────────────────────
router.get('/settings', authenticateToken, adminOnly, async (_req, res) => {
    try {
        const settings = await getSmtpSettings();
        if (!settings)
            return res.json({ settings: { smtp_host: '', smtp_port: 587, smtp_user: '', smtp_pass: '', smtp_secure: 'starttls', from_name: '', from_email: '', enabled: 0 } });
...
router.put('/settings', authenticateToken, adminOnly, async (req, res) => {
    try {
        ...
        if (smtp_host !== undefined)
            update.smtp_host = String(smtp_host).trim();
        if (smtp_port !== undefined)
            update.smtp_port = Number(smtp_port) || 587;
        if (smtp_user !== undefined)
            update.smtp_user = String(smtp_user).trim();
        // Only update password if it's not the masked placeholder
        if (smtp_pass !== undefined && smtp_pass !== '••••••••')
            update.smtp_pass = String(smtp_pass);
        if (smtp_secure !== undefined && ['none', 'tls', 'starttls'].includes(smtp_secure))
            update.smtp_secure = smtp_secure;
        if (from_name !== undefined)
            update.from_name = String(from_name).trim();
        if (from_email !== undefined)
            update.from_email = String(from_email).trim();
...
router.post('/settings/test', authenticateToken, adminOnly, async (_req, res) => {
    try {
...
router.post('/settings/test-send', authenticateToken, adminOnly, async (req, res) => {
    try {
        const { to } = req.body;
        if (!to)
            return res.status(400).json({ ok: false, message: 'Recipient email is required' });
        const settings = await getSmtpSettings();
...
// ── List email accounts ─────────────────────────────────────────
router.get('/accounts', authenticateToken, adminOnly, async (_req, res) => {
    try {
...
router.post('/accounts', authenticateToken, adminOnly, async (req, res) => {
    try {
        const domain = getMailDomain();
        const localPart = String(req.body.local_part || '').trim().toLowerCase().replace(/[^a-z0-9._-]/g, '');
        const displayName = String(req.body.display_name || '').trim();
        if (!localPart)
            return res.status(400).json({ message: 'Local part (before @) is required' });
        if (localPart.length > 64)
            return res.status(400).json({ message: 'Local part too long (max 64 chars)' });
        ...
        const { insertId } = await run('INSERT INTO email_accounts (email, display_name) VALUES (?, ?)', [email, displayName || localPart]);
...
router.delete('/accounts/:id', authenticateToken, adminOnly, async (req, res) => {
    try {
...
router.get('/accounts/:id/emails', authenticateToken, adminOnly, async (req, res) => {
    try {
        const accountId = Number(req.params.id);
        const direction = req.query.direction === 'outbound' ? 'outbound' : 'inbound';
        const limit = Math.min(Math.max(Number(req.query.limit || 50), 1), 200);
        const emails = await query(`SELECT id, sender, recipient, subject, text_body, html_body, direction, is_read, message_id, created_at
       FROM emails
       WHERE account_id = ? AND direction = ?
       ORDER BY created_at DESC
       LIMIT ${limit}`, [accountId, direction]);
        res.json({ emails });
    }
    catch (err) {
...
// ── Get single email ────────────────────────────────────────────
router.get('/emails/:id', authenticateToken, adminOnly, async (req, res) => {
    try {
        const id = Number(req.params.id);
        const email = await get(`SELECT e.*, ea.email AS account_email, ea.display_name AS account_name
       FROM emails e
...
// ── Delete email ────────────────────────────────────────────────
router.delete('/emails/:id', authenticateToken, adminOnly, async (req, res) => {
    try {
        const id = Number(req.params.id);
