...
const coachOrAdmin = (req, res, next) => {
    if (!['coach', 'admin', 'moderator'].includes(req.user?.role))
...
router.get('/overview', authenticateToken, adminOrMod, async (_req, res) => {
    try {
...
router.get('/campaigns', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        ...
        const campaigns = await query(`SELECT c.*, u.name AS coach_name, COUNT(s.id) AS ad_set_count, COALESCE(SUM(a.impressions),0) AS impressions, COALESCE(SUM(a.clicks),0) AS clicks
       FROM ad_campaigns c
...
router.post('/campaigns', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        ...
        const result = await run(`INSERT INTO ad_campaigns (coach_id, name, objective, daily_budget, lifetime_budget, budget_type, schedule_start, schedule_end, status, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, [req.user.id, name, objective || 'coaching', daily_budget || 0, lifetime_budget || 0, budget_type || 'daily', schedule_start || null, schedule_end || null, 'pending_review']);
...
router.put('/campaigns/:id', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const existing = await get('SELECT * FROM ad_campaigns WHERE id = ?', [req.params.id]);
        if (!existing)
            return res.status(404).json({ message: 'Campaign not found' });
        if (req.user.role !== 'admin' && existing.coach_id !== req.user.id)
...
router.get('/ads', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        ...
        const ads = await query(`SELECT a.*, ac.media_url, ac.format, s.name AS ad_set_name, c.name AS campaign_name
       FROM ads a
...
router.patch('/campaigns/:id/action', authenticateToken, adminOrMod, async (req, res) => {
    try {
        ...
        // record an audit row
        await run(`INSERT INTO ad_audit_logs (actor_id, actor_role, action, entity_type, entity_id, old_state, new_state) VALUES (?, ?, ?, 'campaign', ?, ?, ?)`, [req.user.id, req.user.role, `manager.${action}`, req.params.id, JSON.stringify({ status: campaign.status }), JSON.stringify({ status: newStatus, note })]);
        const updated = await get('SELECT * FROM ad_campaigns WHERE id = ?', [req.params.id]);
        res.json({ campaign: updated });
    }
    catch (err) {
...
router.get('/audit', authenticateToken, adminOrMod, async (req, res) => {
    try {
        ...
        const page = Math.max(1, Math.min(10_000, parseInt(req.query.page) || 1));
        const limit = 50;
        const offset = (page - 1) * limit;
        const logs = await query(`SELECT l.*, u.name AS actor_name
...
//# sourceMappingURL=adsManagerRoutes.js.map