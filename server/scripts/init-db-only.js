...
async function main() {
    ...
    const before = await query("SELECT COUNT(*) AS n FROM information_schema.tables WHERE table_schema = DATABASE()");
    console.log(`Tables before: ${before[0]?.n ?? '?'}`);
    await initDatabase();
    const after = await query("SELECT COUNT(*) AS n FROM information_schema.tables WHERE table_schema = DATABASE()");
...
//# sourceMappingURL=init-db-only.js.map