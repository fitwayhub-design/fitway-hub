...
async function getSettings(): Promise<Record<string, string>> {
  if (Date.now() - _settingsCacheTs < 60_000) return _settingsCache;
  try {
...
async function setting(dbKey: string, envVar: string, fallback = ''): Promise<string> {
...
function httpPost(hostname: string, path: string, headers: Record<string, string>, body: object): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(
      ...
      (res) => { let d = ''; res.on('data', c => d += c); res.on('end', () => { try { resolve(JSON.parse(d)); } catch { reject(new Error('Bad JSON: ' + d.slice(0, 200))); } }); }
    );
    req.on('error', reject);
    req.write(payload);
    req.end();
...
//  PAYPAL HELPERS
// ══════════════════════════════════════════════════════════════════════════════
async function paypalHost(): Promise<string> {
  return (await ENV.paypalMode()) === 'live' ? 'api-m.paypal.com' : 'api-m.sandbox.paypal.com';
}

let ppTokenCache: { token: string; exp: number } | null = null;
...
async function ppCall(method: string, path: string, body?: object): Promise<any> {
  const token = await paypalToken();
  const ppHost = await paypalHost();
  if (method === 'GET') {
    return new Promise((resolve, reject) => {
      const r = https.request({ hostname: ppHost, path, method: 'GET', headers: { Authorization: `Bearer ${token}` } },
...
async function egpToUsd(egp: number): Promise<number> {
  const fixed = process.env.EGP_USD_RATE;
  if (fixed && Number(fixed) > 0) return Math.round(egp / Number(fixed) * 100) / 100;
  if (rateCache && Date.now() - rateCache.ts < 6 * 3_600_000) return Math.round(egp / rateCache.rate * 100) / 100;
  try {
    const r = await new Promise<any>((resolve, reject) => {
      const req = https.request({ hostname: 'open.er-api.com', path: '/v6/latest/USD', method: 'GET' },
        (res) => { let d = ''; res.on('data', c => d += c); res.on('end', () => { try { resolve(JSON.parse(d)); } catch { reject(new Error('Bad JSON')); } }); });
      req.on('error', reject); req.end();
    });
    if (r?.rates?.EGP) { rateCache = { rate: r.rates.EGP, ts: Date.now() }; return Math.round(egp / r.rates.EGP * 100) / 100; }
  } catch {}
  return Math.round(egp / 50.5 * 100) / 100;
}

// PayPal Payouts (send money to coach PayPal email)
async function paypalPayout(coachId: number, amountEGP: number, recipientEmail: string, ref: string): Promise<{ ok: boolean; batchId?: string; error?: string }> {
  ...
  try {
    ...
    if (r.batch_header?.payout_batch_id) {
      await run('INSERT INTO credit_transactions (user_id, amount, type, reference_id, description) VALUES (?,?,?,?,?)',
...
async function activateSubscription(userId: number, coachId: number, planCycle: string, planType: string, amountEGP: number, paymentMethod: string, txRef: string) {
  ...
  await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)',
    [coachId, 'new_subscription', '🎉 New Subscriber!', `A new athlete just subscribed. You earned ${coachCut} EGP.`, '/coach/profile']);
}

/** Activate premium for a user */
async function activatePremium(userId: number, planCycle: string, amountEGP: number, paymentMethod: string, txRef: string) {
 ...
 */
async function autoPayout(coachId: number, amount: number, withdrawalId: number): Promise<{ ok: boolean; ref?: string; error?: string; queuedManual?: boolean }> {
  const user = await get<any>('SELECT payment_method_type, payment_phone, payment_wallet_type, paypal_email FROM users WHERE id=?', [coachId]);
...
router.get('/config', async (_req, res) => {
  ...
  } catch { res.status(500).json({ message: 'Failed to load config' }); }
});
 ...
 */
router.post('/intention', authenticateToken, async (req: any, res: Response) => {
  ...
  const user = await get<any>('SELECT id, name, email FROM users WHERE id=?', [req.user.id]);
 ...
 */
router.post('/paypal/capture', authenticateToken, async (req: any, res: Response) => {
  ...
  try {
    ...
    const tx = await get<any>('SELECT * FROM paymob_transactions WHERE paymob_order_id=?', [orderId]);
...
async function verifyPayPalWebhookSig(req: Request): Promise<boolean> {
...
router.post('/webhook/paypal', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (type === 'PAYMENT.CAPTURE.COMPLETED') {
      ...
      if (!orderId) return;
      const tx = await get<any>('SELECT * FROM paymob_transactions WHERE paymob_order_id=? AND status=?', [orderId, 'pending']);
      if (!tx) return;
  ...
  } catch (err: any) { console.error('PayPal webhook error:', err.message); }
});
    ...
    autoPayout(req.user.id, amount, insertId)
      .then(async (result) => {
        if (result.ok && !result.queuedManual) {
          await run('UPDATE withdrawal_requests SET status=?, processed_at=NOW() WHERE id=?',
            ['approved', insertId]);
          await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)',
 ...
 */
router.get('/withdraw/:id', authenticateToken, async (req: any, res: Response) => {
  try {
    const wr = await get<any>('SELECT * FROM withdrawal_requests WHERE id=? AND coach_id=?', [req.params.id, req.user.id]);
    if (!wr) return res.status(404).json({ message: 'Not found' });
    res.json({ withdrawal: wr });
  } catch { res.status(500).json({ message: 'Failed to fetch withdrawal' }); }
});
 ...
 */
export async function autoRefundSubscription(subscriptionId: number): Promise<boolean> {
  try {
    const sub = await get<any>('SELECT * FROM coach_subscriptions WHERE id=?', [subscriptionId]);
    if (!sub) return false;
    ...
    return true;
  } catch (err: any) { console.error('Auto-refund error:', err.message); return false; }
}
...
router.get('/balance', authenticateToken, async (req: any, res: Response) => {
  ...
  } catch { res.status(500).json({ message: 'Failed to fetch balance' }); }
});
