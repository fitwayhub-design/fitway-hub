...
router.patch('/profile', authenticateToken, async (req: any, res: any) => {
  try {
    ...
    if (name !== undefined) {
      const current = await get<any>('SELECT name, role, name_changed FROM users WHERE id = ?', [userId]);
      ...
      if (nameIsChanging) { fields.push('name = ?'); values.push(newName); markNameChanged = true; }
    }
    if (markNameChanged)                 { fields.push('name_changed = 1'); }
    ...
    const updated = await get(
      'SELECT id, name, email, role, avatar, is_premium, coach_membership_active, points, steps, step_goal, height, weight, gender, fitness_goal, activity_level, computed_activity_level, target_weight, weekly_goal, date_of_birth, onboarding_done, name_changed, avg_daily_steps, streak_days, created_at FROM users WHERE id = ?',
      [userId]
    );
 ...
 */
router.get('/public-profile/:id', authenticateToken, async (req: any, res: any) => {
  try {
    const target = Number(req.params.id);
    if (!Number.isFinite(target)) return res.status(400).json({ message: 'invalid id' });
    const user = await get<any>('SELECT id, name FROM users WHERE id = ?', [target]);
    if (!user) return res.status(404).json({ message: 'Not found' });
    const posts = await query(
      `SELECT id, content, media_url, hashtags, likes, created_at
