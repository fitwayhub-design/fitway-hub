...
router.get('/reviews/:coachId', authenticateToken, async (req, res) => {
    try {
...
router.post('/reviews', authenticateToken, async (req, res) => {
    ...
    if (!coachId || !rating || !text?.trim())
        return res.status(400).json({ message: 'Coach ID, rating, and text are required' });
...
router.post('/reports', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const existing = await get(`SELECT id FROM coach_reports
       WHERE coach_id = ? AND user_id = ? AND status = 'pending' AND created_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR)
       LIMIT 1`, [coachId, req.user.id]);
        if (existing) {
            return res.status(400).json({ message: 'You already sent a recent pending report for this coach' });
        }
        await run('INSERT INTO coach_reports (coach_id, user_id, reason, details) VALUES (?,?,?,?)', [coachId, req.user.id, String(reason).trim().slice(0, 120), details ? String(details).trim().slice(0, 3000) : null]);
        const admins = await query("SELECT id FROM users WHERE role = 'admin'");
        for (const admin of admins) {
            const nTitle = '🚩 New Coach Report';
            const nBody = `A user reported coach ${coach.name || '#' + coachId}.`;
            await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [admin.id, 'coach_report', nTitle, nBody, '/admin/coach-reports']);
...
router.get('/coaches', authenticateToken, async (_req, res) => {
    try {
...
router.post('/profile', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (existing) {
            await run('UPDATE coach_profiles SET bio=?, specialty=?, location=?, price=?, available=?, plan_types=?, monthly_price=?, yearly_price=? WHERE user_id=?', [bio || '', specialty || '', location || '', price || 50, available ? 1 : 0, planTypes || 'complete', monthlyPrice || 0, yearlyPrice || 0, req.user.id]);
        }
        else {
            await run('INSERT INTO coach_profiles (user_id, bio, specialty, location, price, available, plan_types, monthly_price, yearly_price) VALUES (?,?,?,?,?,?,?,?,?)', [req.user.id, bio || '', specialty || '', location || '', price || 50, available ? 1 : 0, planTypes || 'complete', monthlyPrice || 0, yearlyPrice || 0]);
        }
...
router.patch('/requests/:id/status', authenticateToken, requireActiveCoachMembershipForDeals, async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        await run('UPDATE coaching_bookings SET status = ?, amount = ?, completed_at = ? WHERE id = ? AND coach_id = ?', [status, status === 'accepted' ? amount : 0, completedAt, id, req.user.id]);
        ...
        await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [booking.user_id, status === 'accepted' ? 'booking_accepted' : 'booking_rejected', notifTitle, notifBody, '/app/coaching']);
...
router.get('/my-coach', authenticateToken, async (req, res) => {
    try {
        const booking = await get(`SELECT cb.*, u.name as coach_name, u.avatar as coach_avatar, u.email as coach_email,
       ...
       ORDER BY cb.created_at DESC LIMIT 1`, [req.user.id]);
        if (!booking)
...
router.post('/gift', authenticateToken, async (req, res) => {
    ...
    if (!coachId || !amount || amount <= 0)
        return res.status(400).json({ message: 'Coach ID and valid amount required' });
    try {
        ...
        if (!sub)
            return res.status(403).json({ message: 'You can only send gifts to coaches you are subscribed to' });
        ...
        // Log transactions
        await run('INSERT INTO credit_transactions (user_id, amount, type, description) VALUES (?, ?, ?, ?)', [coachId, amount, 'gift', `Gift from user #${req.user.id}: ${message || 'No message'}`]);
        await run('INSERT INTO point_transactions (user_id, points, reason, reference_type) VALUES (?, ?, ?, ?)', [req.user.id, -amount, `Gift to coach #${coachId}`, 'gift']);
        // Notify coach
        ...
        await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?, ?, ?, ?, ?)', [coachId, 'gift_received', nTitle, nBody, '/coach/profile']);
...
router.get('/certification', authenticateToken, async (req, res) => {
    try {
        ...
        const pendingReq = await get('SELECT id, status, national_id_url, certification_url, admin_notes, created_at, reviewed_at FROM certification_requests WHERE coach_id = ? ORDER BY created_at DESC LIMIT 1', [req.user.id]);
        res.json({
...
router.post('/certification/subscribe', authenticateToken, upload.fields([
    ...
    try {
        ...
        const existing = await get("SELECT id FROM certification_requests WHERE coach_id = ? AND status = 'pending'", [req.user.id]);
        if (existing) {
        ...
        // Log transaction
        await run('INSERT INTO credit_transactions (user_id, amount, type, description) VALUES (?, ?, ?, ?)', [req.user.id, -fee, 'certification', `Certified Coach request - ${fee} EGP`]);
        // Create certification request (pending admin review)
        await run('INSERT INTO certification_requests (coach_id, status, national_id_url, certification_url, amount_paid) VALUES (?, ?, ?, ?, ?)', [req.user.id, 'pending', nationalIdUrl, certificationUrl, fee]);
        // Notify all admins
        const admins = await query("SELECT id FROM users WHERE role = 'admin'");
        for (const admin of admins) {
            const nTitle = '📋 New Certification Request';
            const nBody = 'A coach has submitted a certification request for review.';
            await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?, ?, ?, ?, ?)', [admin.id, 'certification_request', nTitle, nBody, '/admin/certifications']);
