...
export function moderatorAreaAllowed(perms, area) {
 ...
 */
export async function getModeratorUserOverride(userId) {
    try {
        const row = await get('SELECT permissions FROM moderator_user_permissions WHERE user_id = ?', [userId]);
        if (row?.permissions) {
...
export async function getEffectiveModeratorPermissions(userId) {
 ...
 */
export async function moderatorCanAccessArea(user, area) {
    if (!user)
        return false;
    if (user.role === 'admin')
        return true;
 ...
 */
export const requireModeratorArea = (area) => async (req, res, next) => {
    ...
    if (moderatorAreaAllowed(perms, area))
        return next();
 ...
 */
export async function logModeratorAction(req, entry) {
    try {
        const user = req.user;
        if (!user?.id)
            return;
        const details = entry.details == null ? null
            : typeof entry.details === 'string' ? entry.details
                : JSON.stringify(entry.details);
        await run(`INSERT INTO moderator_audit_log (actor_id, actor_role, area, action, target_type, target_id, details)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [
            user.id,
            user.role || 'unknown',
            entry.area || null,
 ...
 */
export const auditModeratorAction = (area, action, targetType, getTargetId) => (req, res, next) => {
    res.on('finish', () => {
