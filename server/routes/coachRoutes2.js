...
import { uploadVideo, optimizeImage, validateVideoSize, uploadToR2 } from '../middleware/upload.js';
...
async function getTargetedAdsForUser(userId, placementFilter, limit = 20) {
    ...
    const viewer = await get(`SELECT gender, date_of_birth, fitness_goal, activity_level, computed_activity_level,
 ...
 * different ad.
 */
async function getTargetedCampaignAdsForUser(userId, placement, limit = 1) {
    const viewer = await get(`SELECT gender, date_of_birth, fitness_goal, activity_level, computed_activity_level
     FROM users WHERE id = ?`, [userId]);
    let viewerAge = null;
    if (viewer?.date_of_birth) {
...
router.get('/ads/public/home', authenticateToken, async (req, res) => {
    try {
        await expireAds();
...
router.get('/ads/public/community', authenticateToken, async (req, res) => {
    try {
        await expireAds();
...
router.post('/ads/audience-estimate', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const { audience_gender, audience_age_min, audience_age_max, audience_goals, audience_activity_levels, target_city, target_radius_km, target_lat, target_lng, } = req.body;
...
router.get('/ads', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
       ...
       WHERE a.coach_id = ? ORDER BY a.created_at DESC`, [req.user.id]);
        res.json({ ads });
    }
    catch {
        res.status(500).json({ message: 'Failed to fetch ads' });
...
router.post('/ads', authenticateToken, coachOrAdmin, requireActiveCoachMembershipForDeals, uploadVideo.fields([{ name: 'image', maxCount: 1 }, { name: 'video', maxCount: 1 }]), validateVideoSize, optimizeImage(), async (req, res) => {
    const { title, description, specialty, cta, highlight, paymentMethod, ad_type, media_type, objective, duration_hours, duration_days, campaign_name, audience_gender, audience_age_min, audience_age_max, audience_goals, audience_activity_levels, budget_type, daily_budget, total_budget, schedule_start, schedule_end, placement, target_city, target_radius_km, target_lat, target_lng, contact_phone, } = req.body;
    ...
    try {
        ...
        const { insertId } = await run(`INSERT INTO coach_ads (
...
router.put('/ads/:id', authenticateToken, coachOrAdmin, requireActiveCoachMembershipForDeals, uploadVideo.fields([{ name: 'image', maxCount: 1 }, { name: 'video', maxCount: 1 }]), validateVideoSize, optimizeImage(), async (req, res) => {
    ...
    try {
        ...
        await run("UPDATE coach_ads SET title=?, description=?, specialty=?, cta=?, highlight=?, image_url=?, video_url=?, payment_method=?, ad_type=?, media_type=?, objective=?, duration_hours=?, duration_days=?, status='pending', updated_at=NOW() WHERE id=?", [safeTitle, safeDescription, safeSpecialty, safeCta, safeHighlight, imageUrl, videoUrl, paymentMethod || 'ewallet', ad_type || existing.ad_type, media_type || existing.media_type, objective || existing.objective, parseInt(duration_hours ?? existing.duration_hours ?? '0'), parseInt(duration_days ?? existing.duration_days ?? '0'), req.params.id]);
...
router.delete('/ads/:id', authenticateToken, coachOrAdmin, requireActiveCoachMembershipForDeals, async (req, res) => {
    try {
...
router.post('/ads/:id/payment', authenticateToken, coachOrAdmin, requireActiveCoachMembershipForDeals, async (req, res) => {
    ...
    try {
        ...
        if (existing) {
            await run('UPDATE ad_payments SET duration_minutes=?, amount=?, payment_method=?, proof_url=?, phone=?, card_last4=?, status=?, updated_at=NOW() WHERE ad_id=?', [duration_minutes, amount, payment_method || 'ewallet', proof_url || null, phone || null, card_last4 || null, 'pending', req.params.id]);
        }
        else {
            await run('INSERT INTO ad_payments (ad_id, coach_id, duration_minutes, amount, payment_method, proof_url, phone, card_last4, status) VALUES (?,?,?,?,?,?,?,?,?)', [req.params.id, req.user.id, duration_minutes, amount, payment_method || 'ewallet', proof_url || null, phone || null, card_last4 || null, 'pending']);
        }
...
router.post('/ads/:id/click', authenticateToken, async (req, res) => {
    try {
...
router.post('/ads/impressions', authenticateToken, async (req, res) => {
    try {
        const { ids } = req.body;
        if (Array.isArray(ids) && ids.length > 0) {
            ...
            ids.forEach((id) => billImpression(id).catch(() => { }));
        }
        res.json({ ok: true });
    }
    catch {
...
router.post('/follow/:coachId', authenticateToken, async (req, res) => {
    try {
...
router.delete('/follow/:coachId', authenticateToken, async (req, res) => {
    try {
...
router.get('/follow/:coachId/status', authenticateToken, async (req, res) => {
    try {
...
router.get('/users', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const users = await query(`SELECT DISTINCT u.id, u.name, u.email, u.avatar, u.points, u.steps,
...
router.get('/users/:id/profile', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const user = await get(`SELECT id, name, email, avatar, height, weight, gender, points, steps, step_goal,
...
router.post('/users/:id/workout-plan', authenticateToken, coachOrAdmin, requireActiveCoachMembershipForDeals, async (req, res) => {
    ...
    let planId;
    if (existing) {
        await run('UPDATE workout_plans SET title=?, description=?, days_per_week=?, exercises=?, coach_id=? WHERE user_id=?', [title, description, days_per_week, exercisesJson, req.user.id, req.params.id]);
        planId = existing.id;
    }
    else {
        const ins = await run('INSERT INTO workout_plans (user_id, coach_id, title, description, days_per_week, exercises) VALUES (?,?,?,?,?,?)', [req.params.id, req.user.id, title, description, days_per_week, exercisesJson]);
        planId = ins?.insertId;
    }
...
router.post('/users/:id/nutrition-plan', authenticateToken, coachOrAdmin, requireActiveCoachMembershipForDeals, async (req, res) => {
    ...
    let planId;
    if (existing) {
        await run('UPDATE nutrition_plans SET title=?, daily_calories=?, protein_g=?, carbs_g=?, fat_g=?, meals=?, notes=?, coach_id=? WHERE user_id=?', [title, daily_calories, protein_g, carbs_g, fat_g, mealsJson, notes, req.user.id, req.params.id]);
        planId = existing.id;
    }
    else {
        const ins = await run('INSERT INTO nutrition_plans (user_id, coach_id, title, daily_calories, protein_g, carbs_g, fat_g, meals, notes) VALUES (?,?,?,?,?,?,?,?,?)', [req.params.id, req.user.id, title, daily_calories, protein_g, carbs_g, fat_g, mealsJson, notes]);
        planId = ins?.insertId;
    }
...
router.get('/stats', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        ...
        const weeklyRows = await query("SELECT date, COUNT(*) AS sessions FROM coaching_bookings WHERE coach_id = ? AND status = 'accepted' AND date >= ? GROUP BY date ORDER BY date ASC", [coachId, weekStartStr]);
...
router.get('/upcoming-sessions', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const today = new Date().toISOString().split('T')[0];
        const sessions = await query(`SELECT cb.*, u.name AS athlete_name, u.avatar AS athlete_avatar
       ...
       ORDER BY cb.date ASC, cb.time ASC LIMIT 10`, [req.user.id, today]);
        res.json({ sessions });
    }
    catch {
...
router.get('/activity', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const coachId = req.user.id;
...
router.get('/dashboard-home', authenticateToken, coachOrAdmin, async (req, res) => {
    try {
        const coachId = req.user.id;
        const [athletes, recentRequests, recentPosts, recentTransactions] = await Promise.all([
            query(`SELECT u.id, u.name, u.avatar, u.email,
         ...
         LIMIT 6`, [coachId]),
            query(`SELECT cb.id, cb.status, cb.date, cb.time, cb.created_at,
         ...
         LIMIT 6`, [coachId]),
            query(`SELECT p.id, p.content, p.media_url, p.created_at,
         ...
         LIMIT 6`),
            query(`SELECT cs.id, cs.status, cs.amount, cs.credited_amount, cs.plan_type, cs.plan_cycle, cs.created_at,
                u.id AS user_id, u.name AS user_name, u.avatar AS user_avatar
         FROM coach_subscriptions cs
...
router.get('/profile/:coachId/posts', authenticateToken, async (req, res) => {
    try {
        const posts = await query(`SELECT p.*, u.name AS user_name, u.avatar AS user_avatar, u.role AS user_role,
...
router.get('/profile/:coachId/videos', authenticateToken, async (req, res) => {
    try {
        const videos = await query("SELECT id, title, description, url, thumbnail, duration, duration_seconds, category, is_premium FROM workout_videos WHERE coach_id = ? AND COALESCE(approval_status, 'approved') = 'approved' AND (is_short IS NULL OR is_short = 0) ORDER BY created_at DESC LIMIT 30", [req.params.coachId]);
        res.json({ videos });
    }
    catch {
...
router.get('/profile/:coachId/shorties', authenticateToken, async (req, res) => {
    try {
        const videos = await query("SELECT id, title, description, url, thumbnail, duration, duration_seconds, width, height FROM workout_videos WHERE coach_id = ? AND COALESCE(approval_status, 'approved') = 'approved' AND is_short = 1 ORDER BY created_at DESC LIMIT 30", [req.params.coachId]);
        res.json({ videos });
    }
    catch {
...
router.get('/profile/:coachId/photos', authenticateToken, async (req, res) => {
    try {
        const photos = await query(`SELECT id, media_url, content, created_at FROM posts
       ...
       ORDER BY created_at DESC LIMIT 50`, [req.params.coachId]);
        res.json({ photos });
    }
    catch {
...
//# sourceMappingURL=coachRoutes2.js.map