...
router.get('/admin/all', authenticateToken, async (req, res) => {
    ...
    if (!isAdmin && !(await moderatorCanAccessArea(req.user, 'tickets_view')))
...
router.post('/', authenticateToken, async (req, res) => {
    try {
       ...
       VALUES (?, ?, ?, ?, ?, 'open', ?, ?, ?, NOW(), NOW())`, [u.id, coach_id, kind || (isWorkout ? 'workout_question' : 'nutrition_question'), subject, body || '', workout_plan_id || null, nutrition_plan_id || null, itemKey || null]);
...
router.get('/:id', authenticateToken, async (req, res) => {
    try {
       ...
       WHERE t.id = ?`, [req.params.id]);
        ...
        if (!(await canSeeTicket(req, ticket)))
            return res.status(403).json({ message: 'Forbidden' });
...
router.get('/plan-comments/list', authenticateToken, async (req, res) => {
    try {
        ...
        const params = [];
        if (workout_plan_id) {
            where.push('workout_plan_id = ?');
            params.push(Number(workout_plan_id));
        }
        if (nutrition_plan_id) {
            where.push('nutrition_plan_id = ?');
            params.push(Number(nutrition_plan_id));
        }
        if (where.length === 0)
       ...
       ORDER BY c.id ASC LIMIT 500`, params);
...
router.post('/plan-comments', authenticateToken, async (req, res) => {
    try {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, 'open', ?, NOW(), NOW())`, [workout_plan_id || null, nutrition_plan_id || null, exercise_key || null, meal_key || null, u.id, u.role, body, parent_id || null]);
...
router.delete('/plan-comments/:id', authenticateToken, async (req, res) => {
    try {
        ...
        if (!row)
            return res.status(404).json({ message: 'Not found' });
        if (row.author_id !== u.id && u.role !== 'admin')
            return res.status(403).json({ message: 'Forbidden' });
...
router.post('/training-events', authenticateToken, async (req, res) => {
    try {
       ...
       VALUES (?, ?, ?, ?, ?, NOW())`, [u.id, coachId, workout_plan_id || null, event_type, payload ? JSON.stringify(payload) : null]);
        if (coachId) {
            const titles = {
        ...
        if (event_type === 'plan_finished' && workout_plan_id) {
            const prior = await get(`SELECT id FROM credit_transactions WHERE user_id = ? AND type = 'plan_finish_bonus' AND reference_id = ? LIMIT 1`, [u.id, workout_plan_id]).catch(() => null);
            if (!prior) {
             ...
             VALUES (?, ?, 'plan_finish_bonus', ?, ?, NOW())`, [u.id, amount, workout_plan_id, `Bonus for finishing plan #${workout_plan_id}`]);
