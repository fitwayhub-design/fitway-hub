...
import { getPosts, createPost, getChallenges, createChallenge, joinChallenge, inviteToChallenge, likePost, unlikePost, getPostComments, addComment, followUser, unfollowUser, requestChat, acceptChatRequest, getChatRequests, getMessages, sendMessage, getUserProfile, deletePost, getTrendingTags } from '../controllers/communityController.js';
...
router.get('/stats', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const [postRow] = await query('SELECT COUNT(*) as cnt FROM posts');
        const [likeRow] = await query('SELECT COUNT(*) as cnt FROM post_likes');
        const [commentRow] = await query('SELECT COUNT(*) as cnt FROM post_comments');
        const [challengeRow] = await query('SELECT COUNT(*) as cnt FROM challenges');
        const [userRow] = await query("SELECT COUNT(DISTINCT user_id) as cnt FROM posts WHERE created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)");
