...
// Add or update steps
router.post('/add', authenticateToken, async (req: any, res: any, next: any) => {
...
router.post('/goal-completed', authenticateToken, async (req: any, res: any) => {
  try {
    ...
    const user = await dbGet<any>('SELECT points FROM users WHERE id = ?', [userId]);
