...
router.get('/ads/public/home', authenticateToken, async (req, res) => {
    try {
        ...
        let campaignAds = [];
        try {
            campaignAds = await query(`SELECT a.id, a.headline AS title, a.body AS description, 
...
router.post('/users/:id/workout-plan', authenticateToken, coachOrAdmin, requireActiveCoachMembershipForDeals, async (req, res) => {
    ...
    if (existing) {
        ...
        await run('INSERT INTO workout_plans (user_id, coach_id, title, description, days_per_week, exercises) VALUES (?,?,?,?,?,?)', [req.params.id, req.user.id, title, description, days_per_week, exercisesJson]);
    }
...
router.post('/users/:id/nutrition-plan', authenticateToken, coachOrAdmin, requireActiveCoachMembershipForDeals, async (req, res) => {
    ...
    if (existing) {
        ...
        await run('INSERT INTO nutrition_plans (user_id, coach_id, title, daily_calories, protein_g, carbs_g, fat_g, meals, notes) VALUES (?,?,?,?,?,?,?,?,?)', [req.params.id, req.user.id, title, daily_calories, protein_g, carbs_g, fat_g, mealsJson, notes]);
    }
