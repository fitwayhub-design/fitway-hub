...
async function upsert(page, type, label, content, sortOrder) {
    const existing = await get('SELECT id FROM website_sections WHERE page = ? AND type = ? LIMIT 1', [page, type]);
    ...
    await run('INSERT INTO website_sections (page, type, label, content, sort_order) VALUES (?, ?, ?, ?, ?)', [page, type, label, JSON.stringify(content), sortOrder]);
