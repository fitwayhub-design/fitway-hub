...
export const getDailySteps = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ message: 'Unauthorized' });
        const today = new Date().toISOString().split('T')[0];
        const entry = await get('SELECT steps, calories_burned, distance_km FROM steps_entries WHERE user_id = ? AND date = ?', [userId, today]);
        if (entry) {
...
export const syncSteps = async (req, res) => {
    try {
        ...
        const existing = await get('SELECT id FROM steps_entries WHERE user_id = ? AND date = ?', [userId, summaryDate]);
        if (existing) {
            await run('UPDATE steps_entries SET steps = ?, calories_burned = ?, distance_km = ? WHERE user_id = ? AND date = ?', [steps, caloriesBurned || null, distanceKm || null, userId, summaryDate]);
            return res.json({ message: 'Steps updated successfully', date: summaryDate, steps });
        }
        const { insertId } = await run('INSERT INTO steps_entries (user_id, date, steps, calories_burned, distance_km, tracking_mode) VALUES (?, ?, ?, ?, ?, ?)', [userId, summaryDate, steps, caloriesBurned || null, distanceKm || null, 'manual']);
