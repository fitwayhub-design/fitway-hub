...
export const listChallenges = async (req: Request, res: Response) => {
  try {
    ...
    let list = rows.map(c => ({
      ...
      goal_label: METRICS[c.goal_metric]?.label || c.goal_metric,
    }));
  ...
  try {
    ...
    res.json({
      challenge: {
        ...
        goal_label: METRICS[c.goal_metric]?.label || c.goal_metric,
        goal_unit: METRICS[c.goal_metric]?.unit || '',
...
export const updateChallenge = async (req: Request, res: Response) => {
  try {
    ...
    if (locked) await recomputeAll(c.id);
...
export const submitEvidence = async (req: Request, res: Response) => {
  try {
    ...
    if (!allowed.includes(method) || !METHODS[method]) return res.status(400).json({ message: 'This verification method is not allowed for this challenge.' });
    ...
    if (method === 'manual_checkin') {
      const dup = await get<any>(`SELECT id FROM challenge_submissions WHERE challenge_id = ? AND participant_id = ? AND method='manual_checkin' AND activity_date = ? AND status <> 'rejected' AND deleted_at IS NULL`, [c.id, p.id, activityDate]);
...
export const getLeaderboard = async (req: Request, res: Response) => {
  try {
    ...
    res.json({ leaderboard: board, scope, goal_label: METRICS[c.goal_metric]?.label, goal_unit: METRICS[c.goal_metric]?.unit, goal_target: c.goal_target });
  } catch {
...
export const getProgress = async (req: Request, res: Response) => {
  try {
    ...
    if (!p) return res.json({ participant: null, submissions: [], rank: null });
    const subs = await query<any>('SELECT * FROM challenge_submissions WHERE participant_id = ? AND deleted_at IS NULL ORDER BY created_at DESC LIMIT 100', [p.id]);
    ...
    res.json({ participant: p, submissions: subs, rank: rk?.rank || null, goal_target: c.goal_target, goal_unit: METRICS[c.goal_metric]?.unit, scoring_model: c.scoring_model });
  } catch {
...
export const finalizeChallenge = async (req: Request, res: Response) => {
  try {
    ...
    for (let i = 0; i < Math.min(winnersCount, ranked.length, podiumKeys.length); i++) {
      ...
      notify(p.user_id, 'challenge_winner', '🏆 You placed!', `You finished #${i + 1} in “${c.title}”.`, '/app/challenges');
    }
