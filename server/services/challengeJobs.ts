...
export async function autoFinalizeEndedChallenges(): Promise<number> {
  ...
  for (const c of rows) {
    try {
      ...
      if (r.ok) { finalized++; console.log(`🏁 Auto-finalized challenge #${c.id} “${c.title}”`); }
    } catch (e: any) {
...
export async function sendDailyGoalReminders(): Promise<number> {
  ...
  const challenges = await query<any>(
    `SELECT DISTINCT c.id, c.title, c.timezone FROM challenges c
  ...
  const dueByUser = new Map<number, string[]>();
  for (const c of challenges) {
    if (localHourInTz(c.timezone) !== REMINDER_LOCAL_HOUR) continue;
  ...
  for (const [userId, titles] of dueByUser) {
    ...
    const already = await get<any>(
      `SELECT id FROM notifications WHERE user_id = ? AND type = 'challenge_reminder' AND DATE(created_at) = CURDATE() LIMIT 1`,
      [userId],
    );
    if (already) continue;
