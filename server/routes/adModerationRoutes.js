...
router.get('/', authenticateToken, adminOrMod, async (req, res) => {
    try {
        const status = req.query.status || 'pending';
        const page = parseInt(req.query.page) || 1;
        const limit = 20;
        const offset = (page - 1) * limit;
        const reviews = await query(`SELECT r.*,
...
router.get('/:id', authenticateToken, adminOrMod, async (req, res) => {
    try {
        const review = await get(`SELECT r.*, c.*, u.name AS coach_name, u.email AS coach_email
       FROM ad_moderation_reviews r
       JOIN ad_campaigns c ON c.id = r.campaign_id
       JOIN users u ON u.id = c.coach_id
       WHERE r.id = ?`, [req.params.id]);
        ...
        const ads = adSetIds.length
            ? await query(`SELECT a.*, ac.media_url, ac.format FROM ads a LEFT JOIN ad_creatives ac ON ac.id = a.creative_id WHERE a.ad_set_id IN (${adSetIds.map(() => '?').join(',')})`, adSetIds)
            : [];
...
router.patch('/:id', authenticateToken, adminOrMod, async (req, res) => {
    try {
        ...
        // Update review
        await run(`UPDATE ad_moderation_reviews
       SET status = ?, reviewer_id = ?, notes = ?, flags = ?, resolved_at = NOW()
       WHERE id = ?`, [reviewStatus, req.user.id, notes || null, flags ? JSON.stringify(flags) : null, req.params.id]);
        // Update campaign status
        await run(`UPDATE ad_campaigns
       SET status = ?, admin_note = ?, reviewed_by = ?, reviewed_at = NOW(), updated_at = NOW()
       WHERE id = ?`, [campaignStatus, notes || null, req.user.id, review.campaign_id]);
        ...
        if (action === 'reject' || action === 'needs_changes' || action === 'flag') {
            ...
            if (campaign?.coach_id) {
                const titles = { reject: '❌ Campaign Rejected', needs_changes: '⚠️ Campaign Needs Changes', flag: '🚩 Campaign Flagged' };
                const reason = notes ? `: ${notes}` : '';
                await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [campaign.coach_id, 'ad_moderation', titles[action] || 'Campaign Update', `Your campaign "${campaign.name || 'Untitled'}" has been ${action === 'reject' ? 'rejected' : action === 'flag' ? 'flagged for review' : 'sent back for changes'}${reason}`, '/coach/ads/my-ads']);
       ...
       VALUES (?, ?, ?, 'campaign', ?, ?, ?)`, [req.user.id, req.user.role, `moderation.${action}`, review.campaign_id,
...
router.post('/auto-scan', authenticateToken, adminOrMod, async (req, res) => {
    try {
        const { campaign_id } = req.body;
        if (!campaign_id)
            return res.status(400).json({ message: 'campaign_id required' });
        ...
        const dupe = await get(`SELECT id FROM ad_campaigns WHERE coach_id = ? AND name = ? AND id != ? LIMIT 1`, [campaign.coach_id, campaign.name, campaign_id]);
        if (dupe)
            autoFlagReasons.push('Duplicate campaign name detected');
        ...
        if (wasFlagged) {
            ...
            await run(`UPDATE ad_moderation_reviews SET status = 'flagged', auto_flagged = 1, auto_flag_reasons = ? WHERE campaign_id = ? AND status = 'pending'`, [JSON.stringify(autoFlagReasons), campaign_id]);
        }
...
router.get('/stats/summary', authenticateToken, adminOrMod, async (_req, res) => {
    try {
       ...
       FROM ad_moderation_reviews`);
        const recent = await query(`SELECT r.*, c.name AS campaign_name, u.name AS coach_name
       FROM ad_moderation_reviews r
       JOIN ad_campaigns c ON c.id = r.campaign_id
       JOIN users u ON u.id = c.coach_id
...
//# sourceMappingURL=adModerationRoutes.js.map