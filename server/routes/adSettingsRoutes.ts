...
router.get('/', authenticateToken, adminOnly, async (_req: any, res: Response) => {
  try {
    const settings = await query<any>('SELECT * FROM admin_ad_settings ORDER BY category, id');
...
router.get('/public', authenticateToken, async (_req: any, res: Response) => {
  try {
    const publicKeys = [
...
router.patch('/:key', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
    const { key } = req.params;
    const { value, reason } = req.body;
    if (value === undefined) return res.status(400).json({ message: 'value is required' });

    const existing = await get<any>('SELECT * FROM admin_ad_settings WHERE setting_key = ?', [key]);
...
router.post('/bulk', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
    ...
    const all = await query<any>('SELECT * FROM admin_ad_settings ORDER BY category, id');
...
router.get('/history', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
...
router.get('/placements', authenticateToken, adminOnly, async (_req: any, res: Response) => {
  try {
...
router.patch('/placements/:key', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
    ...
    await run(
      `UPDATE ad_placements SET
         ...
         updated_at = NOW()
       WHERE placement_key = ?`,
      [enabled !== undefined ? (enabled ? 1 : 0) : null, max_ads, priority_order, frequency_cap_hours, label, description, req.params.key],
    );
    const p = await get<any>('SELECT * FROM ad_placements WHERE placement_key = ?', [req.params.key]);
...
router.get('/feature-flags', authenticateToken, adminOnly, async (_req: any, res: Response) => {
  try {
    const flags = await query<any>('SELECT * FROM ad_feature_flags ORDER BY id');
    res.json({ flags });
  } catch (err: any) {
    res.status(500).json({ message: err.message });
  }
...
router.patch('/feature-flags/:key', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
    ...
    const flag = await get<any>('SELECT * FROM ad_feature_flags WHERE flag_key = ?', [req.params.key]);
    res.json({ flag });
  } catch (err: any) {
    res.status(500).json({ message: err.message });
  }
...
router.get('/approval-rules', authenticateToken, adminOnly, async (_req: any, res: Response) => {
  try {
    const rules = await query<any>('SELECT * FROM ad_approval_rules ORDER BY priority DESC');
    res.json({ rules });
  } catch (err: any) {
    res.status(500).json({ message: err.message });
  }
...
router.post('/approval-rules', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
    ...
    const rule = await get<any>('SELECT * FROM ad_approval_rules WHERE id = ?', [result.insertId]);
...
router.patch('/approval-rules/:id', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
    const { rule_name, rule_type, conditions, enabled, priority } = req.body;
    await run(
      `UPDATE ad_approval_rules SET
       ...
       WHERE id=?`,
      [rule_name, rule_type, conditions ? JSON.stringify(conditions) : null, enabled !== undefined ? (enabled ? 1 : 0) : null, priority, req.params.id],
    );
    const rule = await get<any>('SELECT * FROM ad_approval_rules WHERE id = ?', [req.params.id]);
    res.json({ rule });
  } catch (err: any) {
    res.status(500).json({ message: err.message });
  }
...
router.delete('/approval-rules/:id', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
...
router.get('/presets', authenticateToken, adminOnly, async (_req: any, res: Response) => {
  try {
...
router.post('/presets', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
    ...
    const preset = await get<any>('SELECT * FROM ad_template_presets WHERE id = ?', [result.insertId]);
...
router.patch('/presets/:id', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
    ...
    await run(
      `UPDATE ad_template_presets SET
       ...
       WHERE id=?`,
      [name, description, config ? JSON.stringify(config) : null, is_default !== undefined ? (is_default ? 1 : 0) : null, target_role, req.params.id],
    );
    const preset = await get<any>('SELECT * FROM ad_template_presets WHERE id = ?', [req.params.id]);
    res.json({ preset });
  } catch (err: any) {
    res.status(500).json({ message: err.message });
  }
...
router.delete('/presets/:id', authenticateToken, adminOnly, async (req: any, res: Response) => {
  try {
...
router.get('/overview', authenticateToken, adminOnly, async (_req: any, res: Response) => {
  try {
    const [campaigns] = await query<any>(`SELECT COUNT(*) AS total, SUM(CASE WHEN status='active' THEN 1 ELSE 0 END) AS active, SUM(CASE WHEN status='pending_review' THEN 1 ELSE 0 END) AS pending FROM ad_campaigns`);
    const [pending_reviews] = await query<any>(`SELECT COUNT(*) AS total FROM ad_moderation_reviews WHERE status='pending'`);
    const [events_today] = await query<any>('SELECT COUNT(*) AS total FROM ad_events WHERE DATE(recorded_at)=CURDATE()');
    const [spend_today] = await query<any>(`SELECT COALESCE(SUM(amount),0) AS total FROM ad_wallet_ledger WHERE entry_type='debit' AND DATE(created_at)=CURDATE()`);
    const flagged = await query<any>(`SELECT * FROM ad_moderation_reviews WHERE status='flagged' LIMIT 5`);
