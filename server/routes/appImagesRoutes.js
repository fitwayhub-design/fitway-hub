...
router.get('/', async (_req, res) => {
    try {
        ...
        const map = {};
        for (const r of rows)
            map[r.slug] = { url: r.url, alt: r.alt, category: r.category };
        res.json({ images: map });
    }
    catch {
        res.json({ images: {} });
...
router.get('/admin/all', authenticateToken, adminOnly, async (_req, res) => {
    try {
...
router.post('/admin/:slug', authenticateToken, adminOnly, upload.single('image'), optimizeImage(), async (req, res) => {
    const { slug } = req.params;
    if (!SLUG_RE.test(slug))
        return res.status(400).json({ message: 'Invalid slug (lowercase, digits, underscore only)' });
    if (!req.file)
    ...
    try {
        ...
        await run(`INSERT INTO app_images (slug, url, alt, category, updated_by)
         VALUES (?,?,?,?,?)
         ON DUPLICATE KEY UPDATE url = VALUES(url), alt = VALUES(alt), category = VALUES(category), updated_by = VALUES(updated_by)`, [slug, url, alt, category, req.user.id]);
...
// ── Admin: delete a slot ──────────────────────────────────────────────────────
router.delete('/admin/:slug', authenticateToken, adminOnly, async (req, res) => {
    const { slug } = req.params;
    if (!SLUG_RE.test(slug))
        return res.status(400).json({ message: 'Invalid slug' });
    try {
...
//# sourceMappingURL=appImagesRoutes.js.map