...
async function hasMailCapableDomain(email) {
    ...
    if (!domain || DISPOSABLE_OR_FAKE_DOMAINS.has(domain))
        return false;
    try {
...
function issueLoginToken(user) {
    ...
    return jwt.sign({ id: user.id, email: user.email }, getJwtSecret(), { algorithm: 'HS256', expiresIn: '30d', issuer: JWT_ISS, audience: JWT_AUD_LOGIN });
}
...
function issueOauthState(provider, mobile = false) {
    return jwt.sign({ provider, nonce: randomUUID(), mobile }, getJwtSecret(), { algorithm: 'HS256', expiresIn: '10m', issuer: JWT_ISS, audience: JWT_AUD_OAUTH });
}
...
async function createOrGetSocialUser(params) {
    ...
    await run('UPDATE users SET name = ?, role = ?, avatar = ?, is_premium = 0, membership_paid = 0 WHERE id = ?', [params.name || email.split('@')[0], 'user', params.avatar || null, user.id]);
...
export const register = async (req, res) => {
    try {
        ...
        if (name)
            await run('UPDATE users SET name = ?, role = ?, is_premium = 0, membership_paid = 0 WHERE id = ?', [name, userRole, user.id]);
        else
            await run('UPDATE users SET role = ?, is_premium = 0, membership_paid = 0 WHERE id = ?', [userRole, user.id]);
...
export const forgotPasswordGetQuestion = async (req, res) => {
    try {
        ...
        if (!user || !user.security_question)
            return res.json({ question: 'Please answer your security question' });
...
export const forgotPasswordVerify = async (req, res) => {
    try {
        ...
        catch { /* table column nullable; non-fatal */ }
    ...
    try {
        ...
        if (token && userId) {
            try {
                ...
                await run('INSERT IGNORE INTO revoked_tokens (user_id, token_hash, expires_at) VALUES (?, ?, DATE_ADD(NOW(), INTERVAL 31 DAY))', [userId, tokenHash]);
...
export const loginWithRememberToken = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: user.id, email: user.email }, getJwtSecret(), { algorithm: 'HS256', expiresIn: '1d', issuer: JWT_ISS, audience: JWT_AUD_LOGIN });
...
export const oauthFacebookStart = async (req, res) => {
...
export const oauthFacebookCallback = async (req, res) => {
    try {
        const code = String(req.query.code || '');
        const state = String(req.query.state || '');
...
export const addOfflineSteps = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ message: 'Unauthorized' });
        ...
        for (const entry of entries) {
            ...
            if (existing) {
                ...
                await run('UPDATE steps_entries SET steps = ?, calories_burned = COALESCE(?, calories_burned), distance_km = COALESCE(?, distance_km), tracking_mode = COALESCE(?, tracking_mode), notes = COALESCE(?, notes) WHERE user_id = ? AND date = ?', [newSteps, caloriesBurned || null, distanceKm || null, trackingMode || null, notes || null, userId, date]);
            }
            else {
                await run('INSERT INTO steps_entries (user_id, date, steps, calories_burned, distance_km, tracking_mode, notes) VALUES (?, ?, ?, ?, ?, ?, ?)', [userId, date, steps, caloriesBurned || null, distanceKm || null, trackingMode || 'manual', notes || null]);
            }
