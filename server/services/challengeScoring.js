...
import { overallGoalsProgress, GOAL_COMPLETION_TARGET } from './challengeGoalTypes.js';
...
export function methodsForType(type) {
    if (type === 'team')
        return ALL_METHODS;
...
export function clampMetric(metricKey, value) {
    const m = METRICS[metricKey] || METRICS.sessions;
    const v = Math.max(0, Number(value) || 0);
    if (v > m.max)
        return { value: m.max, clamped: true };
    return { value: v, clamped: false };
}
// ── Base points for one submission ───────────────────────────────────────────
const BASE_POINTS = 10;
export function basePointsForSubmission(method) {
...
export function aliasForParticipant(p) {
    if (p.display_mode === 'alias' && p.alias)
        return p.alias;
 ...
 */
export async function recomputeParticipant(challengeId, participantId) {
    const challenge = await get('SELECT * FROM challenges WHERE id = ?', [challengeId]);
    if (!challenge)
        return;
    const model = challenge.scoring_model || 'consistency';
    const goals = await query('SELECT * FROM challenge_goals WHERE challenge_id = ? ORDER BY position ASC, id ASC', [challengeId]);
    ...
    const subs = await query(`SELECT * FROM challenge_submissions
     WHERE challenge_id = ? AND participant_id = ? AND deleted_at IS NULL
     ORDER BY activity_date ASC, id ASC`, [challengeId, participantId]);
    ...
    for (const s of approved) {
        if (Number(s.metric_value) > 0) {
            baseline = Number(s.metric_value);
            break;
        }
    }
    ...
    if (goalBased) {
        const overall = overallGoalsProgress(goals, subs);
        ...
        switch (model) {
            ...
            case 'consistency':
                ...
                break;
            case 'participation':
                progress = approved.length;
                break;
            case 'improvement':
                progress = baseline && baseline > 0 ? Math.max(0, ((maxMetric - baseline) / baseline) * 100) : 0;
                break;
...
export async function recomputeAll(challengeId) {
    const parts = await query(`SELECT id FROM challenge_participants WHERE challenge_id = ? AND status IN ('active','left')`, [challengeId]);
    for (const p of parts)
        await recomputeParticipant(challengeId, p.id);
}
/** Whether a participant has met the challenge goal (for completion badges). */
export function hasCompleted(model, progress, goalTarget) {
    if (!goalTarget || goalTarget <= 0)
        return false;
    return progress >= goalTarget;
}
//# sourceMappingURL=challengeScoring.js.map