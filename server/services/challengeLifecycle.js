 ...
 */
export async function finalizeChallengeById(challengeId) {
    ...
    const ranked = await query(`SELECT cp.*, u.name AS user_name FROM challenge_participants cp JOIN users u ON cp.user_id = u.id
     WHERE cp.challenge_id = ? AND cp.status='active'
     ORDER BY cp.verified_points DESC, cp.progress_value DESC, cp.best_streak DESC, cp.joined_at ASC, cp.user_id ASC`, [c.id]);
    const grant = async (p, kind, label, category, tier, points, expiresDays) => {
        const expires = expiresDays > 0 ? new Date(Date.now() + expiresDays * 86400000) : null;
        await run(`INSERT INTO challenge_reward_grants (challenge_id, participant_id, user_id, kind, label, category, tier, points_awarded, expires_at) VALUES (?,?,?,?,?,?,?,?,?)`, [c.id, p.id, p.user_id, kind, label, category, tier, points, expires]);
        if (points > 0) {
    ...
    for (let i = 0; i < Math.min(winnersCount, ranked.length, podiumKeys.length); i++) {
        const p = ranked[i];
        if ((Number(p.verified_points) || 0) <= 0)
            break; // no points → no trophy
    ...
    if (consistent && consistent.best_streak > 0 && tiers.most_consistent)
        await grant(consistent, 'badge', tiers.most_consistent.label || 'Most Consistent', 'most_consistent', null, Number(tiers.most_consistent.points) || 0, Number(tiers.most_consistent.expires_days) || 0);
    if (c.scoring_model === 'improvement') {
        const improved = [...ranked].sort((a, b) => b.progress_value - a.progress_value)[0];
        if (improved && improved.progress_value > 0 && tiers.most_improved)
            await grant(improved, 'badge', tiers.most_improved.label || 'Most Improved', 'most_improved', null, Number(tiers.most_improved.points) || 0, Number(tiers.most_improved.expires_days) || 0);
    }
    // Completion + participation badges.
    for (const p of ranked) {
        const completed = goalTarget > 0 && Number(p.progress_value) >= goalTarget;
        if (completed && tiers.completion)
            await grant(p, 'badge', tiers.completion.label || 'Finisher', 'completion', null, Number(tiers.completion.points) || 0, Number(tiers.completion.expires_days) || 0);
        else if (Number(p.verified_count) > 0 && tiers.participation)
            await grant(p, 'badge', tiers.participation.label || 'Participant', 'participation', null, Number(tiers.participation.points) || 0, Number(tiers.participation.expires_days) || 0);
        if (!p.is_winner)
            notify(p.user_id, 'challenge_completed', 'Challenge finished', `“${c.title}” has ended. See your final rank and rewards.`, '/app/challenges');
    }
...
//# sourceMappingURL=challengeLifecycle.js.map