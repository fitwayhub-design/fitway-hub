...
function isWalletPhoneValid(walletType, phone) {
    ...
    if (!/^\d{11}$/.test(clean))
        return false;
    if (walletType === 'vodafone')
        return clean.startsWith('010');
    if (walletType === 'orange')
        return clean.startsWith('012');
    if (walletType === 'we')
        return clean.startsWith('011');
    return false;
}
function pickWalletPhone(user) {
    const walletType = String(user?.payment_wallet_type || 'vodafone');
    if (walletType === 'orange')
        return user?.payment_phone_orange || user?.payment_phone || null;
    if (walletType === 'we')
        return user?.payment_phone_we || user?.payment_phone || null;
    return user?.payment_phone_vodafone || user?.payment_phone || null;
}
// ── EGP → USD conversion ─────────────────────────────────────────────────────
let cachedRate = null;
const RATE_TTL = 6 * 3600_000; // cache 6 hours
async function egpToUsd(egpAmount) {
...
async function paypalRequest(clientId, secret, method, path, body) {
    const hostname = await getPayPalHostname();
...
router.post('/paypal/capture-order', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (capture.status === 'COMPLETED') {
           ...
           VALUES (?,?,?,?,?,'active','approved','accepted','paypal',NOW(),?,?,NOW())`, [req.user.id, coachId, planCycle, coach.plan_types || 'complete', subAmount,
                ...
                await run('INSERT INTO credit_transactions (user_id, amount, type, description) VALUES (?,?,?,?)', [coachId, coachCutAmt, 'subscription_income', `PayPal subscription from ${payerName}`]);
                await run('INSERT INTO payments (user_id, type, plan, amount, payment_method, transaction_id, status) VALUES (?,?,?,?,?,?,?)', [req.user.id, 'coach_subscription', plan, subAmount, 'paypal', orderId, 'completed']);
                await Promise.all([
                    run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [req.user.id, 'subscription', '✅ Subscription Active!', `Your ${planCycle} plan with the coach is now active.`, '/app/coaching']),
                    run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [coachId, 'subscription', '💰 New Subscriber!', `A user subscribed via PayPal. ${coachCutAmt} EGP added to your balance.`, '/coach/profile']),
                ]);
            ...
            await run('INSERT INTO payments (user_id, type, plan, amount, payment_method, transaction_id, status) VALUES (?,?,?,?,?,?,?)', [req.user.id, type === 'coach' ? 'coach_membership' : 'premium', plan, amount, 'paypal', orderId, 'completed']);
            if (type === 'coach') {
...
router.post('/ewallet', authenticateToken, uploadPaymentProof, optimizeImage(), async (req, res) => {
    ...
    try {
        ...
        await run('INSERT INTO payments (user_id, type, plan, amount, payment_method, proof_url, wallet_type, sender_number, status) VALUES (?,?,?,?,?,?,?,?,?)', [req.user.id, type === 'coach' ? 'coach_membership' : 'premium', plan, amount, 'ewallet', proofUrl, walletType, senderNumber, 'pending']);
        res.json({
...
router.patch('/approve/:paymentId', authenticateToken, async (req, res) => {
...
router.patch('/reject/:paymentId', authenticateToken, async (req, res) => {
    if (req.user?.role !== 'admin')
        return res.status(403).json({ message: 'Admin only' });
    const { paymentId } = req.params;
    ...
    try {
        ...
        if (payment?.user_id) {
            ...
            await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [payment.user_id, 'payment_rejected', '❌ Payment Rejected', `Your e-wallet payment of ${payment.amount || ''} EGP has been rejected${note}`, '/app/coaching']);
...
router.post('/booking/paypal/create-order', authenticateToken, async (req, res) => {
...
router.post('/booking/paypal/capture', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const capture = await paypalRequest(clientId, secret, 'POST', `/v2/checkout/orders/${orderId}/capture`, {});
        if (capture.status === 'COMPLETED') {
            await run('UPDATE coaching_bookings SET payment_status = ?, payment_transaction_id = ?, payment_method = ? WHERE id = ? AND user_id = ?', ['paid', orderId, 'paypal', bookingId, req.user.id]);
...
router.post('/booking/ewallet', authenticateToken, upload.single('proof'), optimizeImage(), async (req, res) => {
    ...
    const proofUrl = await uploadToR2(req.file, 'proofs');
    try {
        await run('UPDATE coaching_bookings SET payment_status = ?, payment_proof = ?, payment_method = ? WHERE id = ? AND user_id = ?', ['proof_submitted', proofUrl, `ewallet_${walletType}`, bookingId, req.user.id]);
...
router.patch('/booking/confirm-payment/:bookingId', authenticateToken, async (req, res) => {
...
router.post('/coach-subscribe', authenticateToken, upload.single('proof'), optimizeImage(), async (req, res) => {
    const { coachId, planCycle, planType, walletType, senderNumber } = req.body;
    if (!coachId || !planCycle || !planType)
        return res.status(400).json({ message: 'Coach ID, plan cycle, and plan type are required' });
    if (!req.file)
        return res.status(400).json({ message: 'Payment proof screenshot is required' });
    if (!walletType || !senderNumber)
        return res.status(400).json({ message: 'Wallet type and sender number are required' });
    try {
        // Get coach's pricing
        const coach = await get(`SELECT u.id, COALESCE(cp.monthly_price, 0) as monthly_price, COALESCE(cp.yearly_price, 0) as yearly_price, COALESCE(cp.plan_types, 'complete') as plan_types
       FROM users u LEFT JOIN coach_profiles cp ON cp.user_id = u.id
...
router.get('/coach-subscriptions', authenticateToken, async (req, res) => {
    if (req.user?.role !== 'admin')
        return res.status(403).json({ message: 'Admin only' });
    try {
...
router.patch('/coach-subscriptions/:id/approve', authenticateToken, async (req, res) => {
    if (req.user?.role !== 'admin')
        return res.status(403).json({ message: 'Admin only' });
    const { id } = req.params;
    try {
        ...
        await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [sub.coach_id, 'subscription', 'New Subscription Request', 'A user paid and requested to subscribe to you. Please accept or decline from Requests.', '/coach/profile']);
        sendPushFromTemplate(sub.coach_id, 'subscription_verified', {}, '/coach/profile').catch(() => { });
        await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [sub.user_id, 'subscription', 'Payment Verified', 'Admin verified your payment. Waiting for coach acceptance.', '/app/coaching']);
...
router.patch('/coach-subscriptions/approve-all', authenticateToken, async (req, res) => {
    if (req.user?.role !== 'admin')
        return res.status(403).json({ message: 'Admin only' });
    try {
        const pendingSubs = await query(`SELECT id, user_id, coach_id
       ...
       ORDER BY created_at ASC`);
        ...
        for (const sub of pendingSubs) {
            await run('UPDATE coach_subscriptions SET status = ?, admin_approval_status = ?, admin_approved_at = NOW() WHERE id = ?', ['pending_coach', 'approved', sub.id]);
            await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [sub.coach_id, 'subscription', 'New Subscription Request', 'A user paid and requested to subscribe to you. Please accept or decline from Requests.', '/coach/profile']);
...
router.patch('/coach-subscriptions/:id/reject', authenticateToken, async (req, res) => {
    if (req.user?.role !== 'admin')
        return res.status(403).json({ message: 'Admin only' });
    const { id } = req.params;
    const { note } = req.body;
    try {
        ...
        const currentStatus = normalizeCoachSubscriptionStatus(sub.status);
        if (currentStatus === 'active')
            return res.status(400).json({ message: 'Active subscription cannot be rejected' });
        if (currentStatus === 'rejected_admin' || currentStatus === 'rejected_by_coach' || currentStatus === 'refunded') {
       ...
       WHERE id = ?`, ['rejected_admin', 'rejected', 'completed', note || 'Rejected by admin', id]);
        await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [sub.user_id, 'refund', 'Subscription Rejected and Refunded', `Your payment for coach subscription was rejected by admin and marked as refunded.${note ? ` Reason: ${note}` : ''}`, '/app/coaching']);
...
router.get('/coach-subscription-requests', authenticateToken, async (req, res) => {
    ...
    try {
       ...
       ORDER BY cs.created_at DESC`, [req.user.id]);
        res.json({ subscriptions: subs });
    }
    catch {
...
router.get('/coach-active-subscriptions', authenticateToken, async (req, res) => {
    if (req.user?.role !== 'coach' && req.user?.role !== 'admin') {
        return res.status(403).json({ message: 'Coach access required' });
    }
    try {
       ...
       ORDER BY cs.expires_at ASC, cs.created_at DESC`, [req.user.id]);
        res.json({ subscriptions: subs });
    }
    catch {
...
router.patch('/coach-subscriptions/:id/coach-accept', authenticateToken, async (req, res) => {
    if (req.user?.role !== 'coach' && req.user?.role !== 'admin') {
        return res.status(403).json({ message: 'Coach access required' });
    }
    const { id } = req.params;
    try {
       ...
       WHERE id = ?`, ['active', 'accepted', coachCut, id]);
        ...
        await run('INSERT INTO credit_transactions (user_id, amount, type, reference_id, description) VALUES (?,?,?,?,?)', [sub.coach_id, coachCut, 'subscription_income', sub.id, `Subscription accepted from ${payerName} (${sub.plan_cycle} ${sub.plan_type})`]);
        await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [sub.user_id, 'subscription', 'Coach Accepted Your Subscription ✅', 'Your coach accepted your subscription and your plan is now active.', '/app/coaching']);
...
router.patch('/coach-subscriptions/:id/coach-decline', authenticateToken, async (req, res) => {
    if (req.user?.role !== 'coach' && req.user?.role !== 'admin') {
        return res.status(403).json({ message: 'Coach access required' });
    }
    const { id } = req.params;
    ...
    try {
       ...
       WHERE id = ?`, ['rejected_by_coach', 'rejected', 'processing', reason || 'Declined by coach', id]);
        ...
        autoRefundSubscription(Number(id))
            ...
            if (!refunded) {
                ...
                await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [sub.user_id, 'refund', 'Subscription Declined — Refund Pending', `The coach declined. Your refund is being processed and will appear soon.${reason ? ' Reason: ' + reason : ''}`, '/app/coaching']);
            }
        })
            .catch(() => { });
        await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [sub.user_id, 'refund', 'Coach Declined Subscription', `Your coach declined the subscription request. If you paid, a refund is being processed automatically.${reason ? ' Reason: ' + reason : ''}`, '/app/coaching']);
...
router.get('/my-subscriptions', authenticateToken, async (req, res) => {
    try {
       ...
       ORDER BY cs.expires_at DESC`, [req.user.id]);
        res.json({ subscriptions: subs });
    }
    catch {
        res.status(500).json({ message: 'Failed to fetch subscriptions' });
...
router.patch('/subscriptions/:id/auto-renew', authenticateToken, async (req, res) => {
...
router.get('/my-credit', authenticateToken, async (req, res) => {
    try {
        ...
        const transactions = await query(`SELECT ct.*, su.name AS payer_name
...
router.get('/payment-info', authenticateToken, async (req, res) => {
    try {
        const user = await get('SELECT payment_method_type, payment_phone, payment_phone_vodafone, payment_phone_orange, payment_phone_we, payment_wallet_type, paypal_email, card_holder_name, card_number, instapay_handle FROM users WHERE id = ?', [req.user.id]);
        if (!user)
...
router.post('/payment-info', authenticateToken, async (req, res) => {
    const { paymentMethodType, paymentPhone, paymentPhoneVodafone, paymentPhoneOrange, paymentPhoneWe, walletType, paypalEmail, cardHolderName, cardNumber, instapayHandle, } = req.body;
    ...
    try {
        ...
        const effectivePhone = wallet === 'orange'
            ...
            : wallet === 'we'
                ? (we || null)
                : (vodafone || null);
        await run('UPDATE users SET payment_method_type = ?, payment_phone = ?, payment_phone_vodafone = ?, payment_phone_orange = ?, payment_phone_we = ?, payment_wallet_type = ?, paypal_email = ?, card_holder_name = ?, card_number = ?, instapay_handle = ? WHERE id = ?', [methodType, effectivePhone, vodafone || null, orange || null, we || null, wallet || null, paypalEmail || null, cardHolderName || null, cardNumber || null, instapayHandle || null, req.user.id]);
...
router.post('/withdraw', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (methodType === 'ewallet' && !selectedPhone)
            return res.status(400).json({ message: 'Please set your e-wallet number first' });
        if (methodType === 'paypal' && !user.paypal_email)
            return res.status(400).json({ message: 'Please set your PayPal email first' });
        if (methodType === 'credit_card' && !user.card_number)
...
router.get('/my-withdrawals', authenticateToken, async (req, res) => {
    try {
        const withdrawals = await query('SELECT * FROM withdrawal_requests WHERE coach_id = ? ORDER BY created_at DESC', [req.user.id]);
...
router.get('/withdrawals', authenticateToken, async (req, res) => {
    if (req.user?.role !== 'admin')
        return res.status(403).json({ message: 'Admin only' });
    try {
        const withdrawals = await query(`SELECT wr.*, u.name as coach_name, u.email as coach_email
...
router.patch('/withdrawals/:id/approve', authenticateToken, async (req, res) => {
    if (req.user?.role !== 'admin')
        return res.status(403).json({ message: 'Admin only' });
    try {
        ...
        if (wr.status !== 'pending')
            return res.status(400).json({ message: `Cannot approve — status is already '${wr.status}'` });
        const { note } = req.body;
        await run('UPDATE withdrawal_requests SET status = ?, admin_note = ?, processed_at = NOW() WHERE id = ? AND status = ?', ['approved', note || '', req.params.id, 'pending']);
        await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [wr.coach_id, 'withdrawal', 'Withdrawal Approved! ✅',
        ...
        await run('INSERT INTO credit_transactions (user_id, amount, type, description) VALUES (?,?,?,?)', [wr.coach_id, -wr.amount, 'withdrawal_approved', `Withdrawal of ${wr.amount} EGP approved by admin`]);
...
router.patch('/withdrawals/:id/reject', authenticateToken, async (req, res) => {
    if (req.user?.role !== 'admin')
        return res.status(403).json({ message: 'Admin only' });
    const { note } = req.body;
    try {
        ...
        if (!wr)
            return res.status(404).json({ message: 'Request not found' });
        if (wr.status !== 'pending')
            return res.status(400).json({ message: `Cannot reject — status is already '${wr.status}'` });
        await run('UPDATE withdrawal_requests SET status = ?, admin_note = ?, processed_at = NOW() WHERE id = ? AND status = ?', ['rejected', note || '', req.params.id, 'pending']);
        // Refund credit
        await run('UPDATE users SET credit = credit + ? WHERE id = ?', [wr.amount, wr.coach_id]);
        await run('INSERT INTO credit_transactions (user_id, amount, type, description) VALUES (?,?,?,?)', [wr.coach_id, wr.amount, 'withdrawal_refund', `Withdrawal rejected: ${note || 'No reason given'}`]);
        await run('INSERT INTO notifications (user_id, type, title, body, link) VALUES (?,?,?,?,?)', [wr.coach_id, 'withdrawal', 'Withdrawal Declined', `Your withdrawal of ${wr.amount} EGP was declined. Credit has been refunded. ${note ? 'Reason: ' + note : ''}`, '/coach/profile']);
