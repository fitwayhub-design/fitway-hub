...
router.post('/upload-branding-image', authenticateToken, adminOnly, upload.single('image'), optimizeImage(), async (req, res) => {
    try {
        if (!req.file)
...
router.post('/upload-dashboard-image', authenticateToken, adminOnly, upload.single('image'), optimizeImage(), async (req, res) => {
    try {
        if (!req.file)
            return res.status(400).json({ message: 'No image file provided' });
