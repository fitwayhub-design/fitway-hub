...
export const saveSession = async (req, res) => {
    try {
        ...
        const { insertId } = await run('INSERT INTO premium_sessions (user_id, start_time, end_time, total_steps, total_distance_km, calories, path_json) VALUES (?, ?, ?, ?, ?, ?, ?)', [userId, toMySQLDatetime(startTime), toMySQLDatetime(endTime), safeSteps, +distKm.toFixed(3), safeCals, JSON.stringify(safePath)]);
        ...
        if (existing) {
            ...
            await run('UPDATE steps_entries SET steps = ?, calories_burned = ?, distance_km = ?, tracking_mode = ? WHERE id = ?', [mergedSteps, mergedCals, +mergedDist.toFixed(3), 'live', existing.id]);
        }
        else {
            await run('INSERT INTO steps_entries (user_id, date, steps, calories_burned, distance_km, tracking_mode) VALUES (?, ?, ?, ?, ?, ?)', [userId, today, safeSteps, safeCals, +distKm.toFixed(3), 'live']);
        }
