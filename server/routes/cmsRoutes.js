...
async function ensureRequiredSections(page) {
    const required = REQUIRED_SECTIONS[page];
    if (!required?.length)
        return;
    for (const r of required) {
        const existing = await get('SELECT id, content FROM website_sections WHERE page = ? AND type = ? LIMIT 1', [page, r.type]);
        if (existing) {
            ...
            if (changed) {
                await run('UPDATE website_sections SET content = ?, updated_at = NOW() WHERE id = ?', [JSON.stringify(existingContent), existing.id]);
            }
            continue;
        }
        const maxOrder = await get('SELECT MAX(sort_order) as max FROM website_sections WHERE page = ?', [page]);
        await run('INSERT INTO website_sections (page, type, label, content, sort_order) VALUES (?,?,?,?,?)', [page, r.type, r.label, JSON.stringify(r.content), (maxOrder?.max || 0) + 1]);
    }
}
...
router.get('/sections/:page', async (req, res) => {
    try {
        ...
        const sections = await query('SELECT * FROM website_sections WHERE page = ? AND is_visible = 1 ORDER BY sort_order ASC', [page]);
...
router.get('/admin/sections/:page', authenticateToken, adminOnly, async (req, res) => {
    try {
        const { page } = req.params;
        await ensureRequiredSections(page);
        const sections = await query('SELECT * FROM website_sections WHERE page = ? ORDER BY sort_order ASC', [page]);
        const parsed = sections.map(s => ({
            ...s,
            content: typeof s.content === 'string' ? JSON.parse(s.content) : s.content,
        }));
...
router.post('/admin/sections', authenticateToken, adminOnly, async (req, res) => {
    const { page, type, label, content, sort_order } = req.body;
    if (!page || !type || !label)
        return res.status(400).json({ message: 'page, type, label required' });
    try {
        const maxOrder = await get('SELECT MAX(sort_order) as max FROM website_sections WHERE page = ?', [page]);
        const order = sort_order ?? ((maxOrder?.max || 0) + 1);
        const { insertId } = await run('INSERT INTO website_sections (page, type, label, content, sort_order) VALUES (?,?,?,?,?)', [page, type, label, JSON.stringify(content || {}), order]);
...
router.put('/admin/sections/:id', authenticateToken, adminOnly, async (req, res) => {
    const { id } = req.params;
...
router.post('/admin/sections/reorder', authenticateToken, adminOnly, async (req, res) => {
...
router.delete('/admin/sections/:id', authenticateToken, adminOnly, async (req, res) => {
    try {
...
router.post('/admin/upload-image', authenticateToken, adminOnly, upload.single('image'), optimizeImage(), async (req, res) => {
    if (!req.file)
