...
function daysAgoStr(n) { return daysAgo(n).toISOString().split('T')[0]; }
...
async function seed() {
    ...
    // ── Admin ──────────────────────────────────────────────────────────────────
    await run(`INSERT INTO users (name,email,password,role,is_premium,points,steps,gender,height,weight) VALUES (?,?,?,?,?,?,?,?,?,?)`, ['Peter Admin', 'peteradmin@example.com', await hash(adminPw), 'admin', 0, 9999, 0, 'male', 178, 80]);
    // ── Coaches ────────────────────────────────────────────────────────────────
    const coachIds = [];
    ...
    for (let i = 0; i < COACH_NAMES.length; i++) {
        ...
        const { insertId: cId } = await run(`INSERT INTO users (name,email,password,role,is_premium,points,steps,gender,height,weight,
       ...
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [name, fakeEmail(name, 100 + i), i === 0 ? await hash(coachPw) : fakeH, 'coach', 1,
        ...
        try {
            await run(`INSERT INTO coach_profiles (user_id, bio, specialty, location, available, plan_types, monthly_price, yearly_price)
         VALUES (?,?,?,?,?,?,?,?)`, [cId,
    ...
    const { insertId: testUserId } = await run(`INSERT INTO users (name,email,password,role,is_premium,points,steps,gender,height,weight,
      date_of_birth,target_weight,weekly_goal,fitness_goal,activity_level,onboarding_done,
      avg_daily_steps,streak_days,step_goal,email_verified)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, ['Test User', 'test@example.com', await hash(userPw), 'user', 1,
    ...
    const FITNESS_GOALS = ['lose_weight', 'build_muscle', 'maintain_weight', 'gain_weight'];
    const ACTIVITY_LEVELS = ['sedentary', 'light', 'moderate', 'active', 'very_active'];
    // Age brackets: 18-24, 25-34, 35-44, 45-54, 55-65 — rotate through evenly
    const AGE_BRACKETS = [[18, 24], [25, 34], [35, 44], [45, 54], [55, 65]];
    for (let i = 0; i < allNames.length; i++) {
        ...
        const { insertId: uId } = await run(`INSERT INTO users (name,email,password,role,is_premium,points,steps,gender,height,weight,
       ...
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [name, fakeEmail(name, i + 1), fakeH, 'user', isPremium,
    ...
    for (const uid of [...userIds, testUserId]) {
        ...
        for (let d = 0; d < 14; d++) {
            ...
            try {
                await run('INSERT INTO steps_entries (user_id,date,steps,calories_burned,distance_km) VALUES (?,?,?,?,?)', [uid, daysAgoStr(d), steps, cal, km]);
            }
            catch { }
        }
    }
    ...
    for (let i = 0; i < userIds.length; i++) {
        ...
        for (let p = 0; p < numPosts; p++) {
            ...
            const { insertId: pid } = await run('INSERT INTO posts (user_id,content,hashtags,likes,created_at) VALUES (?,?,?,?,?)', [uid, body, '#fitness #fitwayhub #health', likes, daysAgo(randInt(0, 30))]);
            postIds.push(pid);
        }
    }
    // Test user posts
    const { insertId: testPost1 } = await run('INSERT INTO posts (user_id,content,hashtags,likes) VALUES (?,?,?,?)', [testUserId, 'Week 1 done! Already feeling stronger and more energetic 💪 Thanks to my FitWay coach!', '#fitnessjourney #fitwayhub', 28]);
    ...
    for (let i = 0; i < coachIds.length; i++) {
        ...
        for (let p = 0; p < numPosts; p++) {
            ...
            const { insertId: pid } = await run('INSERT INTO posts (user_id,content,hashtags,likes,created_at) VALUES (?,?,?,?,?)', [cid, body, '#coaching #fitness #fitwayhub #Egypt', likes, daysAgo(randInt(0, 21))]);
            postIds.push(pid);
        }
    }
    ...
    for (const pid of commentTargets) {
        ...
        for (let c = 0; c < numComments; c++) {
            ...
            try {
                await run('INSERT INTO post_comments (post_id,user_id,content,created_at) VALUES (?,?,?,?)', [pid, commenter, pick(COMMENT_BODIES), daysAgo(randInt(0, 10))]);
            }
            catch { }
        }
    }
    ...
    for (const uid of premiumUsers) {
        await run('INSERT INTO payments (user_id,type,plan,amount,payment_method,status,created_at) VALUES (?,?,?,?,?,?,?)', [uid, 'premium', 'monthly', 50, pick(['vodafone_cash', 'orange_cash', 'paypal', 'paymob_card']), 'completed', daysAgo(randInt(1, 60))]);
    }
    await run('INSERT INTO payments (user_id,type,plan,amount,payment_method,status) VALUES (?,?,?,?,?,?)', [testUserId, 'premium', 'monthly', 50, 'paymob_card', 'completed']);
    ...
    for (let i = 0; i < Math.min(userIds.length, 14); i++) {
        ...
        try {
            await run(`INSERT INTO coach_subscriptions (user_id,coach_id,plan_cycle,plan_type,amount,status,admin_approval_status,coach_decision_status,payment_method,started_at,expires_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)`, [uid, cid, pick(['monthly', 'monthly', 'yearly']), pick(['complete', 'workout']), amount, status,
            ...
            if (status === 'active') {
                ...
                await run('INSERT INTO credit_transactions (user_id,amount,type,description) VALUES (?,?,?,?)', [cid, credit, 'subscription_earning', `Subscription from user #${uid}`]);
            }
        }
        catch { }
    }
    try {
        await run(`INSERT INTO coach_subscriptions (user_id,coach_id,plan_cycle,plan_type,amount,status,admin_approval_status,coach_decision_status,payment_method) VALUES (?,?,?,?,?,?,?,?,?)`, [testUserId, mainCoach, 'monthly', 'complete', 149, 'active', 'approved', 'accepted', 'paymob_card']);
    }
    catch { }
    ...
    for (let i = 0; i < Math.min(8, userIds.length); i++) {
        ...
        for (let j = 0; j < template.length; j++) {
            const isCoach = j % 2 !== 0;
            await run('INSERT INTO messages (sender_id,receiver_id,content,created_at) VALUES (?,?,?,?)', [isCoach ? cid : uid, isCoach ? uid : cid, template[j], daysAgo(7 - j * 0.5)]);
        }
    }
    for (const [msg, fromCoach] of [
        ...
        await run('INSERT INTO messages (sender_id,receiver_id,content) VALUES (?,?,?)', [fromCoach ? mainCoach : testUserId, fromCoach ? testUserId : mainCoach, msg]);
    }
    ...
    for (const cid of coachIds) {
        ...
        for (let i = 0; i < numCampaigns; i++) {
            ...
            const campColsRes = await query(`SHOW COLUMNS FROM ad_campaigns`);
            const campCols = Array.isArray(campColsRes) ? campColsRes.map((r) => r.Field) : [];
            const cCols = ['coach_id', 'name', 'objective', 'status'];
            const cVals = [cid, campName, objective, status];
            ...
            let campaignId;
            try {
                const res = await run(campSql, cVals);
                ...
                try {
                    ...
                    if (idx !== -1) {
                        const cols2 = cCols.slice();
                        const vals2 = cVals.slice();
                        cols2.splice(idx, 1);
                        vals2.splice(idx, 1);
                        const sql2 = `INSERT INTO ad_campaigns (${cols2.join(',')}) VALUES (${cols2.map(_ => '?').join(',')})`;
                        const res2 = await run(sql2, vals2);
                    ...
                    const res3 = await run(`INSERT INTO ad_campaigns (coach_id,name,created_at,updated_at) VALUES (?,?,?,?)`, [cid, campName, start, end]);
         ...
         VALUES (?,?,?,?,?,?,?)`, [cid, 'coach', 'create', 'campaign', campaignId, JSON.stringify({ name: campName, status }), start]).catch(() => { });
            totalAuditLogs++;
            ...
            for (let s = 0; s < numAdSets; s++) {
                ...
                const adSetColsRes = await query(`SHOW COLUMNS FROM ad_sets`);
                const adSetCols = Array.isArray(adSetColsRes) ? adSetColsRes.map((r) => r.Field) : [];
                const adSetColTypes = Array.isArray(adSetColsRes) ? adSetColsRes.reduce((acc, r) => { acc[r.Field] = r.Type; return acc; }, {}) : {};
                ...
                totalAdSets++;
                await run(`INSERT INTO ad_audit_logs (actor_id, actor_role, action, entity_type, entity_id, new_state, created_at)
           VALUES (?,?,?,?,?,?,?)`, [cid, 'coach', 'create', 'ad_set', adSetId, JSON.stringify({ name: adSetName, status: adSetStatus }), start]).catch(() => { });
                totalAuditLogs++;
                ...
                for (let a = 0; a < numAds; a++) {
                    ...
                    try {
                        ...
                        const acCols = Array.isArray(acColsRes) ? acColsRes.map((r) => r.Field) : [];
                        ...
                        if (acCols.includes('name')) {
                            cols.push('name');
                            vals.push(`${campName} creative`);
                        }
                        if (acCols.includes('format')) {
                            cols.push('format');
                            vals.push(creativeType);
                        }
                        if (acCols.includes('media_url')) {
                            cols.push('media_url');
                            vals.push(creativeUrl);
                        }
                        ...
                        if (cols.length > 0) {
                            ...
                            const minCols = ['name', 'media_url', 'created_at'];
                            const minVals = [`${campName} creative`, creativeUrl, adStart];
                            if (acCols.includes('owner_id')) {
                                minCols.unshift('owner_id');
                                minVals.unshift(cid);
                            }
                            else if (acCols.includes('coach_id')) {
                                minCols.unshift('coach_id');
                                minVals.unshift(cid);
                            }
                            else if (acCols.includes('created_by')) {
                                minCols.unshift('created_by');
                                minVals.unshift(cid);
                            }
                            const res = await run(`INSERT INTO ad_creatives (${minCols.join(',')}) VALUES (${minCols.map(_ => '?').join(',')})`, minVals);
                            creativeId = res.insertId;
                        }
                        // expose creativeId for next insertion
                        global.__lastCreativeId = creativeId;
                        totalCreatives++;
                    }
                    catch (e) {
                        console.warn('Creative insert error:', e.message);
                        throw e;
                    }
                    ...
                    const creativeIdToUse = global.__lastCreativeId || null;
                    // Insert ad using available columns on this DB
                    const adColsRes = await query(`SHOW COLUMNS FROM ads`);
                    const adCols = Array.isArray(adColsRes) ? adColsRes.map((r) => r.Field) : [];
                    const aCols = [];
                    const aVals = [];
                    ...
                    if (adCols.includes('creative_id')) {
                        ...
                        aVals.push(creativeIdToUse);
                    }
                    if (adCols.includes('placement')) {
                        aCols.push('placement');
                        aVals.push(pick(adPlacements));
                    }
                    ...
                    const { insertId: adId } = await run(`INSERT INTO ads (${aCols.join(',')}) VALUES (${aCols.map(_ => '?').join(',')})`, aVals);
                    totalAds++;
                    await run(`INSERT INTO ad_audit_logs (actor_id, actor_role, action, entity_type, entity_id, new_state, created_at)
             VALUES (?,?,?,?,?,?,?)`, [cid, 'coach', 'create', 'ad', adId, JSON.stringify({ name: adName, status: adStatus }), adStart]).catch(() => { });
                    totalAuditLogs++;
                }
            }
        }
    ...
    for (let i = 0; i < BLOGS.length; i++) {
        ...
        try {
            await run(`INSERT INTO blog_posts (title,slug,language,excerpt,content,status,author_id,author_role,published_at) VALUES (?,?,?,?,?,?,?,?,?)`, [blog.title, blog.slug, 'en', blog.excerpt, blog.content, 'published', authorId, 'coach', daysAgo(randInt(1, 90))]);
        }
        catch (e) {
            console.warn('Blog seed error:', e.message);
        }
    }
    console.log('✅ Seeded 8 blog posts');
    // ── 30-Day Challenge ───────────────────────────────────────────────────────
    const { insertId: challengeId } = await run('INSERT INTO challenges (creator_id,title,description,start_date,end_date) VALUES (?,?,?,?,?)', [mainCoach, '30-Day Transformation Challenge',
    ...
    for (const uid of [...userIds.slice(0, 10), testUserId]) {
        try {
            await run('INSERT INTO challenge_participants (challenge_id,user_id) VALUES (?,?)', [challengeId, uid]);
        }
        catch { }
    }
