...
export async function requestOtp(email: string, purpose: OtpPurpose): Promise<{ ok: boolean; message: string; cooldown?: number }> {
  ...
  const recent = await get<OtpRow>(
    `SELECT id, created_at FROM email_otps
     WHERE email = ? AND purpose = ?
       AND created_at > DATE_SUB(NOW(), INTERVAL ? SECOND)
     ORDER BY id DESC LIMIT 1`,
    [normalized, purpose, OTP_RESEND_COOLDOWN_SECONDS]
  );
  if (recent) {
  ...
  await run(
    `INSERT INTO email_otps (email, code_hash, purpose, attempts, used, expires_at)
     VALUES (?, ?, ?, 0, 0, DATE_ADD(NOW(), INTERVAL ? SECOND))`,
    [normalized, codeHash, purpose, OTP_TTL_SECONDS]
  );
...
export async function verifyOtp(email: string, code: string, purpose: OtpPurpose): Promise<{ ok: boolean; message: string }> {
  const normalized = String(email || '').trim().toLowerCase();
  const cleanCode = String(code || '').trim();
  if (!normalized || !cleanCode) return { ok: false, message: 'Email and code are required' };

  const row = await get<OtpRow>(
  ...
  if (!row) return { ok: false, message: 'No code requested. Please request a new code.' };
  if (row.used) return { ok: false, message: 'This code has already been used. Please request a new code.' };
...
function buildEmail(code: string, purpose: OtpPurpose): { subject: string; text: string; html: string } {
  const purposeLabel =
  ...
  const subject =
    purpose === 'register' ? 'Verify your email — FitWay Hub' :
    purpose === 'forgot_password' ? 'Password reset code — FitWay Hub' :
    'Password change code — FitWay Hub';

  const text =
    `Your FitWay Hub verification code is: ${code}\n\n` +
