...
import { createBlog, deleteBlog, getBlogs, getPublicBlogBySlug, getPublicBlogs, updateBlog, } from '../controllers/blogController.js';
const router = Router();
...
router.post('/', authenticateToken, uploadVideo.fields([
    { name: 'headerImage', maxCount: 1 },
    { name: 'video', maxCount: 1 },
]), validateVideoSize, optimizeImage(), createBlog);
router.put('/:id', authenticateToken, uploadVideo.fields([
    { name: 'headerImage', maxCount: 1 },
    { name: 'video', maxCount: 1 },
]), validateVideoSize, optimizeImage(), updateBlog);
...
router.put('/:id/review', authenticateToken, async (req, res) => {
    try {
        ...
        const updated = await get(`SELECT bp.*, u.name AS author_name, u.avatar AS author_avatar FROM blog_posts bp LEFT JOIN users u ON u.id = bp.author_id WHERE bp.id = ?`, [postId]);
