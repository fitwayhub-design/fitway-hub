...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    if (!otp) return res.status(400).json({ message: 'Email verification code is required' });
    ...
    if (!otpResult.ok) return res.status(400).json({ message: otpResult.message });
...
export const forgotPasswordOtpReset = async (req: Request, res: Response) => {
  try {
    const email = normalizeEmail(req.body?.email);
    ...
    try { await UserModel.setRememberToken(user.id, null as any); } catch { /* non-fatal */ }
...
export const requestChangePasswordOtp = async (req: any, res: Response) => {
  try {
