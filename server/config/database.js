...
export async function initDatabase() {
    ...
    try {
        await run("ALTER TABLE coach_subscriptions ADD COLUMN package_id VARCHAR(40) DEFAULT 'basic'");
    }
    catch { }
    try {
        await run("ALTER TABLE coach_subscriptions ADD COLUMN tickets_used INT DEFAULT 0");
    }
    catch { }
    ...
    try {
        await run("ALTER TABLE workout_videos ADD COLUMN training_id INT DEFAULT NULL");
    }
    catch { }
    try {
        await run("ALTER TABLE workout_videos ADD INDEX idx_workout_videos_training (training_id)");
    }
    catch { }
