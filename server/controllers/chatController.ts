...
export const sendMessage = async (req: Request, res: Response) => {
  try {
    ...
    if (receiverId) {
      sendPushFromTemplate(Number(receiverId), 'new_message', { name: (newMessage as any)?.sender_name || 'Someone' }).catch(() => {});
    }
...
export const getContacts = async (req: Request, res: Response) => {
  try {
    ...
    markOnline(userId);
    const userRole = (req as any).user.role;
    const supportOnly = String((req as any).query?.supportOnly || '') === '1';
    let contacts: any[] = [];
